...
コンポーネントおよびユーザー管理パックは、2019 年Skype for Business Server使用されます。 Skype for Business Server 2019 と Skype for Business Server 2015 の両方がインストールされている共存シナリオの場合は、引き続き Skype for Business Server 2015 管理パックを使用する必要があります。Skype for Business Server 2015 コンピューター。
...
- **System Center Operations Manager** このメソッドを使用すると、Operations Manager を使用して、サーバーの監視をSkype for Business Server。

- **Operations Manager シェル** Operations Manager シェル を使用して直接インポートしたり、管理パックをインポートするときに発生する問題をトラブルシューティングするには、System Center Operations Manager コンソールを使用します。
...
2. [操作System Center] で、[管理] を **クリックします**。
...
6. [インポートする **管理パック** の選択] ダイアログ ボックスで、ファイルを検索して選択し、[Microsoft.LS.2019.Monitoring.ActiveMonitoring.mp を Microsoft.LS.2019.Monitoring.ComponentAndUser.mp 開く] をクリック **します**。 ダイアログ ボックスで複数のファイルを選択するには、最初のファイルをクリックし、Ctrl キーを押しながら後続のファイルをクリックします。
...
1. [**スタート]** ボタンを **クリックし、[** すべてのプログラム] **をクリックし、[Microsoft System Center 2012] を** クリックし、[Operations **Manager**] をクリックし、[プログラム] **Operations Manager シェル**。
