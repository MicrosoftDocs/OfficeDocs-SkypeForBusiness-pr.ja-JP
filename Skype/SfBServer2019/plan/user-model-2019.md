...
|:-----|:-----|
|16 個のフロントエンド サーバーとバック エンド サーバーを備え、フロントエンド プール、または高可用性を実現する SQL常時オンのバック エンド サーバーのペア。  <br/> |106,000 人の一意のユーザーが同時にログインし、モバイル 以外のインスタンスを表す 50% の複数のプレゼンス (MPOP) に加えて、合計 210,000 のエンドポイントで Mobility を有効にしたユーザーの 40% が表示されます。  <br/> |
|音声ビデオ会議  <br/> |フロントエンド プールによって提供される音声ビデオ会議サービスは、最大会議サイズが 250 ユーザーであり、一度に実行される大規模な会議は 1 つのみと仮定して、プールの会議をサポートします。  <br/> **注:** さらに、2 台のフロント エンド サーバーを備える独立したフロント エンド プールを展開して大規模な会議をホストすることで、250 ~ 1000 人の大規模な会議をサポートできます。 詳細については、「大規模な会議[を計画する」を参照Skype for Business Server。](../../SfbServer/plan-your-deployment/conferencing/large-meetings.md) <br/> |
