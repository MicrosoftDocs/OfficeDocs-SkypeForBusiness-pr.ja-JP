...
Skype for Business の音声は、EF 46 ("AF41") キューに展開する必要があります。 Skype for business のビデオは、34 () キューに展開する必要があります。 これは、Microsoft 365 または Office 365 の電話システムまたはその他のテレフォニー機能が展開されているかどうかに関係なく、ピアツーピアと会議のトラフィックにも当てはまります。
...
QoS の設定と管理の詳細については、「[サービスの品質の管理](https://technet.microsoft.com/library/gg425841.aspx)」をご覧ください。

## <a name="bypass-proxies-and-wan-optimization-devices"></a>プロキシと WAN 最適化デバイスをバイパスする

Skype for Business Online を含むすべての Microsoft 365 または Office 365 は暗号化され、通常はプロキシデバイスによって検査することはできません。 このような理由から、ユーザーが[office 365 url と IP アドレス範囲](https://support.office.com/article/8548a211-3fe7-47cb-abb1-355ea5aa88a2)に対して行う接続として定義されているすべての Microsoft 365 および office 365 ネットワークトラフィックに対してプロキシデバイスをバイパスすることをお勧めします。 プロキシ デバイスはリアルタイムの Skype for Business Online メディア ストリームでは遅延を引き起こす可能性があるため、それらのトラフィックではプロキシ デバイスを可能な限りバイパスすることをお勧めします。

Microsoft は、PAC ファイルを使用して microsoft 365 および Office 365 の Url を除外して、Microsoft 365 および Office 365 トラフィックをファイアウォールに送信することをお勧めします。

次のいくつかのリソースに記載されている情報もご覧ください。

- [ベースラインとパフォーマンス履歴を使用した Microsoft 365 または Office 365 のパフォーマンスのチューニング](https://support.office.com/article/1492cb94-bd62-43e6-b8d0-2a61ed88ebae)
...
- [Microsoft 365 または Office 365 での WAN 最適化コントローラーまたはトラフィック/検査デバイスの使用](https://aka.ms/kb2690045)

- [Microsoft 365 または Office 365 向け ExpressRoute でのルーティング](https://support.office.com/article/e1da26c6-2d39-4379-af6f-4da213218408)

## <a name="bypass-double-encryption"></a>二重暗号化をバイパスする

オーディオとビデオを最大限に活用できるように、Skype for Business メディア (音声およびビデオ) が仮想プライベートネットワーク (VPN) トンネルを通過できないようにするソリューションを実装する必要があります。 すべての Skype for Business トラフィックは、トランスポート層セキュリティ (TLS) を使って暗号化され、メディアワークロードは安全なリアルタイムプロトコル (SRTP) で暗号化されます。 シグナリングは TLS を使用して暗号化され、メディアのワークロードは SRTP で暗号化されます。 このトラフィックを VPN トンネル経由で送信すると、暗号化の追加レイヤーが追加され、クライアントと Microsoft 365 または Office 365 の間にネットワークホップが追加されます。これにより、ジッタ、パケット損失、待機時間が増加するため、パフォーマンスが低下する可能性があります。
...
> [!NOTE]
> これは、Skype for Business のメディアのワークロードにのみ必要であり、他の Microsoft 365 や Office 365 サービスには適用されません。
...
- [Enable Direct Access (DirectAccess を有効にする)](https://technet.microsoft.com/library/jj574163.aspx)

## <a name="ensure-the-right-ports-and-protocols-are-open"></a>正しいポートとプロトコルが開いていることを確認する

お客様は、Microsoft 365 または Office 365 サービスに必要な Url と IP アドレスの到達可能性を確保する必要があります。 Skype for Business Online のすべての IP アドレスと URL のリストについては、「[Office 365 URL および IP アドレス範囲](https://support.office.com/article/8548a211-3fe7-47cb-abb1-355ea5aa88a2)」をご覧ください。
...
- [Skype for Business の電話とデバイス](https://technet.microsoft.com/office/dn947482.aspx)

- [個人用周辺機器のソリューション カタログ](http://partnersolutions.skypeforbusiness.com/solutionscatalog/personal-peripherals-pcs)

- [Microsoft Lync で認定済みの電話とデバイス](https://technet.microsoft.com/office/dn788944.aspx)
