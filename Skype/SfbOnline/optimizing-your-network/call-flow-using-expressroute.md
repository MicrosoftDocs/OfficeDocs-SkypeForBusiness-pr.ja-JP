...
Microsoft 365 または Office 365、Skype for Business Server Hybrid、または Skype for Business Cloud Connector Edition の一部として Skype for Business Online を展開する場合、Skype for Business Online サービスを効果的に計画、展開、運用、トラブルシューティングするために、Skype for Business クライアントとサーバー間の通信とコール フローを理解する必要があります。
 ...
 **ExpressRoute** これは、Microsoft ネットワークへの専用のプライベート接続を提供する、ネットワーク全体の一部であるネットワーク セグメントです。 これは、Skype for Business Online のリアルタイム通信など、ネットワークの速度とパフォーマンスに依存しているすべてのワークロードについて、ネットワークを Microsoft ネットワーク (Microsoft 365 または Office 365 データセンター) に接続する場合に推奨されるオプションです。 ネットワークと Microsoft ネットワーク間の ExpressRoute 接続は [、ExpressRoute](/azure/expressroute/expressroute-locations) 接続プロバイダーを使用してプライベートネットワークと管理対象ネットワークを提供します。99.9% のアップタイムとサービス品質 (QoS) のサポートにより、ネットワークの混雑期間中のリアルタイム メディアのパフォーマンスを向上させることができます。

 **Microsoft ネットワーク** これは、Microsoft 365 および Office 365 サービスをサポートするネットワーク全体の一部です。 これには、Microsoft 365 または Office 365 用のオンライン サーバー間のすべての通信が含まれます。 Microsoft ネットワーク バックボーンをトラバースし、地理的地域間で送信されるトラフィックが含まれる可能性があります。
...
このトラフィックの宛先は [、Office Microsoft 365](https://support.office.com/article/8548a211-3fe7-47cb-abb1-355ea5aa88a2) または Office 365 サービスの 365 URL と IP アドレス範囲で確認できます。 URL ごとに、トラフィックのその部分が Microsoft 365 向け ExpressRoute または 365 用 ExpressRoute をトラバースOffice示します。 ExpressRoute が有効な場合、インターネットが一部のトラフィックに引き続き使用されている図については、Office [365](https://support.office.com/article/6d2534a2-c19c-4a99-be5e-33a0cee5d3bd)向け Azure ExpressRoute を参照してください。 ExpressRoute 経由でルーティング可能としてリストされる URL がインターネット経由でもルーティング可能であることを理解することは重要です。 つまり、一部のシナリオでは、インターネットまたは ExpressRoute が使用されるかどうかに関する決定は、クライアントの場所と、プロキシ サーバーおよびファイアウォールの構成に依存します。 また、Microsoft 365 または Office 365 に関連付けられているすべての URL が ExpressRoute を使用できないので、ExpressRoute パートナーから ExpressRoute を購入した場合でもインターネット接続が必要です。

インターネット経由でのみ送信できるトラフィックには、証明書失効リスト (CRL)、DNS 参照と名前解決などの一般的なインターネット依存関係、Microsoft 365 管理センターなどの共有 Microsoft 365 または Office 365 サービスの URL、Skype コンシューマーとの相互運用性のためのテレメトリやフェデレーションなど、Skype for Business Online の一部の非リアルタイム通信機能が含まれます。の他に、Skype 会議ブロードキャスト用にストリーミングされるメディアも含まれます。 意思決定に役立てるため、ネットワーク ルーティング計画時の考慮事項について、「[Office 365 向け ExpressRoute でのルーティング](https://support.office.com/article/e1da26c6-2d39-4379-af6f-4da213218408)」を参照してください。
...
1. Skype for Business 会議は、会議開催者の自宅がある場所と同じ地域でホストされます。 これは、開催者がオンライン ユーザーの場合はクラウド内、会議開催者がオンプレミス ユーザーの場合はオンプレミス データセンター内にあります。
...
一般に、一方のエンドポイントがネットワーク内に存在し、もう一方のエンドポイントが Microsoft 365 または Office 365 データセンターにある場合、ネットワーク トラフィックは ExpressRoute 接続をトラバースします。 これには、クライアントとサーバー間のシグナリング トラフィック、電話会議中に使用されるメディア トラフィック、オンライン エッジ サーバーを使用するピアツーピア通話が含まれます。

両方のエンドポイントがインターネットで直接通信可能である場合、またはネットワーク内に配置されている場合、トラフィックは ExpressRoute 接続をトラバースしません。 これには、ピアツーピア通話用のメディア、インターネットからオンプレミス展開へのトラフィック、またはインターネットと Microsoft 365 または Office 365 エッジ サーバー間のトラフィックが含まれます。 この例としては、ホテルからオンライン会議に参加するユーザーが挙げられます。
...
### <a name="peer-to-peer-call-for-microsoft-365-or-office-365-user-from-within-customer-network"></a>顧客ネットワーク内から Microsoft 365 または Office 365 ユーザーに対するピアツーピア通話
<a name="bk_Figure2"> </a>

ピアツーピア通話の場合、メディア トラフィックは常にその宛先への最短ルートを取ります。 ただし、シグナリング トラフィックは、オンライン ユーザーが自宅である Microsoft 365 Office 365 データセンターに送信されます。 両方のユーザーが同じ WAN 上で、クライアントが直接通信することを妨げるものは何もないため、メディアは両ユーザー間で直接やり取りします。 シグナリング トラフィックは、両方のユーザーに対し組織のデータセンターごとに送信される ExpressRoute 接続をトラバースします。 このシナリオでのコール フローを確認するには、以下を参照してください。
...
ピアツーピアの例では、メディア トラフィックは常に宛先への最も直接的なルートを受け取っています。 ただし、オンライン会議の場合、宛先はクラウド内に保存されます。 つまり、ネットワーク内から会議に参加しているすべてのユーザーのメディア トラフィックは ExpressRoute 接続をトラバースし、シグナリング トラフィックはクラウドに移動します。 次の図は、メディアとシグナリングの両方がネットワーク内のユーザーの ExpressRoute 接続をトラバースし、喫茶店やホテルなど、ネットワークの外部からインターネットに接続されているユーザーのインターネットを直接トラバースする場合を示しています。

会議の場所は出席者ではなく、会議開催者によって定義されます。 つまり、会議がオンプレミスのお客様によってスケジュールされた場合、メディア トラフィックは ExpressRoute を通してクラウドに流れ込むのではなく、インターネットをトラバースして会議開催者のオンプレミス データセンターに移動します。

オンライン会議用メディアの宛先は、Microsoft 365 または Office 365 クラウド内のデータセンターになりますが、データセンターが会議に参加しているユーザーとは異なる地理的領域にある可能性があります。 これは、次のいずれかの場合に発生する可能性があります。
...
ホストされている会議をサポートする会議サーバーは、会議開催者の自宅がある場所によって決定されます。 このシナリオでは、ハイブリッド展開のオンプレミス ユーザーによってスケジュールされた会議に参加しているすべてのユーザーのメディアは、オンプレミス データセンターに向かいます。 Online 自宅ユーザー向けのシグナリングは、クラウド内の組織を通じて確立され、メディアは直接接続を試みる。 このシナリオでは両方のユーザーがネットワーク内から接続し、直接メディア通信が可能なため、ExpressRoute はオンライン自宅ユーザー用のシグナリング トラフィックにのみ使用されます。 オンライン自宅ユーザーがインターネットから接続する場合、接続にオンライン エッジ サーバーが使用される場合はメディアは ExpressRoute をトラバースできます。

 **ハイブリッド ユーザー コール フローによってホストされる会議**

![Call flow hosted onprem.](../images/9e669859-19f3-4a86-95b7-7185eb421ccd.png)

### <a name="on-premises-edge-server-with-microsoft-365-or-office-365-hosted-conferences"></a>Microsoft 365 または Office 365 でホストされる会議を含むオンプレミス エッジ サーバー
<a name="bk_Figure5"> </a>

ハイブリッド ユーザーがオンラインでホストされる会議に参加すると、シグナリングとメディアは Microsoft 365 または Office 365 クラウド宛てに送信され、ユーザーはインターネットから参加しているので、通常は直接インターネット パスが取られます。 ただし、場合によっては、ファイアウォール制限などのために直接インターネット パスが使用できないことがあります。 この場合、オンプレミスエッジ サーバーはメディア トラフィックを中継できます。これにより、メディア トラフィックは ExpressRoute 回線をクラウドにトラバースする前にオンプレミス ネットワークに戻る原因になります。
 ...
 **Microsoft 365 または Office 365 および Cloud Connector の電話システムを介した PSTN 通話**
...
Skype 会議ブロードキャストは特別な使用例で、各部分が異なるネットワークトランスポート プロファイルを持つ 2 部構成の会議で構成されます。 最初の部分と、ネットワーク パフォーマンスの観点から最も重要なものは、内部会議です。 これは、クラウド内の会議サーバーに接続する 1 つ以上のクライアント エンドポイントを含む会議のリアルタイム部分です。 会議のこの部分を使用して送信されるデータは上記の例とまったく同じで、ユーザーがオンライン会議に参加しています。
...
Skype for Business Online の使用シナリオには、自宅オンラインのユーザーが関係し、内部ネットワークまたはインターネットから通話している可能性があります。 オンプレミス サーバーはこのようなシナリオの一部ではないので、すべての会議または PSTN 関連メディアがクラウドに流れ、Online ユーザーのエッジ サーバーもクラウド内に表示されます。
...
|ピアツーピア通話  <br/> |2 つのクライアント、両方ともにネットワーク上。  <br/> |ExpressRoute  <br/> |ローカル  <br/> |[顧客ネットワーク内から Microsoft 365 または Office 365 ユーザーに対するピアツーピア通話](call-flow-using-expressroute.md#bk_Figure2) <br/> ||
|ピアツーピア通話  <br/> |2 つのクライアント、1 つはネットワーク上 (内部)、もう 1 つはインターネット上 (外部)。  <br/> |内部ユーザー: ExpressRoute  <br/> 外部ユーザー: インターネット  <br/> |内部ユーザー: ExpressRoute  <br/> 外部ユーザー: インターネットから Microsoft 365 または Office 365 Edge サーバー。  <br/> |[顧客ネットワーク内から Microsoft 365 または Office 365 ユーザーに対するピアツーピア通話](call-flow-using-expressroute.md#bk_Figure2) <br/> |ファイアウォールがクライアント間の直接接続をブロックするとします。これにはオンライン エッジ サーバーが必要です。内部ユーザーからオンライン エッジ サーバーへのトラフィックは、電話会議用の会議サーバーへのパスと似たパスをたどります。  <br/> |
...
|インターネットのユーザーによる電話会議への参加  <br/> |クライアントは、インターネットとクラウドの会議サーバー上にいます。  <br/> |インターネット  <br/> |インターネット  <br/> |[オンラインでホストされている会議に参加しているネットワーク上のオンライン ユーザー](call-flow-using-expressroute.md#bk_Figure3) <br/> ||
...
|PSTN 通話  <br/> |顧客ネットワークのクライアントとクラウド内の電話システム サーバー  <br/> |ExpressRoute  <br/> |ExpressRoute  <br/> |[オンラインでホストされている会議に参加しているネットワーク上のオンライン ユーザー](call-flow-using-expressroute.md#bk_Figure3) <br/> ||
|PSTN 通話  <br/> |インターネット上のクライアントとクラウド内の電話システム サーバー  <br/> |インターネット  <br/> |インターネット  <br/> |該当しない  <br/> |メディアとシグナリングは、Microsoft 365 または Office 365 データセンターに流れ込まれます。 クライアント エンドポイントがインターネット上にあるため、すべてのデータはインターネットで Microsoft データセンターに向かいます (オンライン エッジ サーバーが接続に必要な場合でも)。  <br/> |
...
|:-----|:-----|:-----|:-----|:-----|:-----|
|**使用シナリオ** <br/> |**エンドポイント** <br/> |**シグナリング パス** <br/> |**メディア パス** <br/> |**サンプル フロー** <br/> |**メモ** <br/> |
|ピアツーピア通話  <br/> |2 つのクライアント、顧客ネットワーク上および自宅オンプレミス上  <br/> |ローカル  <br/> |ローカル  <br/> |[顧客ネットワーク内から Microsoft 365 または Office 365 ユーザーに対するピアツーピア通話](call-flow-using-expressroute.md#bk_Figure2) <br/> |ユーザーはオンプレミスに自宅に配置されているので、シグナリングはクラウドではなくオンプレミス データセンターにローカルに流れます。  <br/> |
|ピアツーピア通話  <br/> |2 つのクライアント、両方が顧客ネットワークから接続 1 つは自宅オンライン、もう 1 つは自宅オンプレミス  <br/> |オンライン ユーザー: ExpressRoute  <br/> オンプレミス ユーザー: ローカル  <br/> |ローカル  <br/> |[顧客ネットワーク内から Microsoft 365 または Office 365 ユーザーに対するピアツーピア通話](call-flow-using-expressroute.md#bk_Figure2) <br/> |シグナリング トラフィックをクラウドに送信するのは、Online 自宅ユーザーのみです。  <br/> |
|フェデレーション関係にある組織のユーザーへのピアツーピア通話  <br/> |2 つのクライアント、1 つは顧客ネットワークのオンプレミス ユーザー (内部)、もう 1 つはフェデレーション関係にある会社のネットワークのオンライン ユーザー (フェデレーション)  <br/> |内部ユーザー: ローカル  <br/> フェデレーション関係にあるユーザー１：ExpressRoute  <br/> |インターネットまたは ExpressRoute (オンラインまたはオンプレミス エッジ サーバーが使用されているかどうかに依存)  <br/> |[ネットワーク上](call-flow-using-expressroute.md#bk_Figure3) のオンライン ユーザーが [、Microsoft 365 または Office 365](call-flow-using-expressroute.md#bk_Figure5) でホストされる会議 (メディア トラフィックの場合) で、オンラインおよびオンプレミス エッジ サーバーの一部としてホストされている会議に参加します。 <br/> |ファイアウォールがクライアント間の直接接続をブロックしていて、オンライン エッジ サーバーが必要であるとします。ICE ネゴシエーションにより接続に対してオンライン (オンライン ユーザーによる) およびオンプレミス エッジサーバ (オンプレミス ユーザーによる) が提供されます。  <br/> |
...
|PSTN 通話  <br/> |Cloud Connector エディションを使用したインターネットを使用したオンライン ユーザー。  <br/> |インターネット  <br/> |インターネット  <br/> |Skype for Business Cloud Connector Edition を使用した [、Microsoft 365 または Office 365](call-flow-using-expressroute.md#bk_Figure5) でホストされる電話会議と PSTN 通話とのオンプレミス エッジ サーバー [の組み合わせ](call-flow-using-expressroute.md#bk_Figure6)。  <br/> |インターネット ユーザーは Cloud Connector に含まれているエッジサーバー経由で接続し、Cloud Connector は PSTN ネットワークに接続します。  <br/> |
