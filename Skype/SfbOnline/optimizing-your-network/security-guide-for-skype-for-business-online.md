...
SfBO では、Microsoft 365 または Office 365 および TLS 内のサーバー間の通信に相互 TLS (MTLS) が使用されます。クライアントからサービスには、特定の会話を攻撃できる期間内にこの攻撃を実現することは非常に困難です。TLS はすべての関係者を認証し、すべてのトラフィックを暗号化します。これにより、盗聴は防止されませんが、暗号化が解除されない限り、攻撃者はトラフィックを読み取ることができません。
...
Azure Active Directory は、Microsoft 365 および Office 365 のディレクトリサービスとして機能します。すべてのユーザーディレクトリ情報とポリシーの割り当てを保存します。 
...
フェデレーションは、他の組織と通信して IM とプレゼンスを共有できるようにします。既定では、SfBO フェデレーションがオンになっています。ただし、テナント管理者は、Microsoft 365 または Office 365 管理ポータルを使用してこれを制御することができます。詳細はこちらをご覧ください。

## <a name="addressing-threats-to-sfbo-conferences"></a>SfBO 会議の脅威に対する取り組み

SfBO は、エンタープライズユーザーがリアルタイムの Web 会議を作成して参加できるようにする機能を提供します。エンタープライズユーザーは、AAD、Microsoft 365、または Office 365 アカウントを持っていない外部ユーザーも、これらの会議に参加するように招待することができます。セキュリティで保護された id を持つフェデレーションパートナーによって採用されたユーザーは、会議に参加することもできます。昇格させた場合は、発表者として使用できます。匿名ユーザーは、発表者として会議を作成したり、会議に参加したりすることはできませんが、参加後に発表者に昇格することはできます。
