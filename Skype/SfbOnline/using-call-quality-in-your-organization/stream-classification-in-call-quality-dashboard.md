...
|**メトリック**|**状態**|**解説**|
|:-----|:-----|:-----|
|オーディオ劣化の平均|> 1.0|ネットワーク平均オピニオン評点によるストリームの平均劣化値。 ビデオ ローカル フレーム 損失率の平均|
|往復|500 ミリ秒より長い|RFC3550 での規定に従って計算されたミリ秒単位の平均ネットワーク伝達ラウンド トリップ時間。|
...
|**手順 #**|**メトリック**|**状態**|**条件が真である場合の分類**|**条件が偽の場合の分類**|**メトリックが利用できない場合の分類**|**解説**|
|:-----|:-----|:-----|:-----|:-----|:-----|:-----|
|1|ビデオ ローカル フレーム 損失率の平均|> 50% |不良|良好|手順 2 に進んでください|ユーザーに対し表示されるビデオ フレームの損失率の平均。 これにはネットワーク損失から回復したフレームも含まれます。|
...
|**手順 #**|**メトリック**|**状態**|**条件が真である場合の分類**|**条件が偽の場合の分類**|**メトリックが利用できない場合の分類**|**解説**|
|:-----|:-----|:-----|:-----|:-----|:-----|:-----|
|1|VBSS ローカルフレーム損失率の平均|> 50% |不良|良好|手順 2 に進んでください|ユーザーに対し表示されるビデオ フレームの損失率の平均。 これにはネットワーク損失から回復したフレームも含まれます。|
|2|VBSSフレーム レートの平均|<2|不良|良好|手順 3 に進んでください|セッション期間で算出された、ビデオ ストリームで受信した秒あたりの平均フレーム数。|
|3|VBSS 投稿 FECPLR|> 0.15|不良|良好|未分類|未分類|すべてのビデオ ストリームおよびコーデックにわたり集計された、FEC が適用された後のパケット損失率。|
...
|アプリケーション共有での相対的な一方向の平均|> 1.75|アプリケーション共有ストリームでの、エンドポイント間の相対的な一方向の平均遅延 (ミリ秒単位)。|
