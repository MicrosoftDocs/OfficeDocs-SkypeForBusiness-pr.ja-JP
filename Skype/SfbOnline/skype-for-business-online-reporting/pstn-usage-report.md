...
*    **型の呼び出し**は、呼び出しが送信または受信の PSTN の呼び出しし、呼び出しの呼び出しなどの型は、ユーザーまたはオーディオ会議で配置したかどうかです。 表示される可能性のある通話の種類は次のとおりです。 
     ...
     *    **注:** この PSTN 使用状況レポートに表示されません、自動アテンダントまたは呼び出しキューなど、UC アプリケーションからユーザーに転送された呼び出し呼び出しレグは、ピア ツー ピア (P2P) の音声通話します。 "ツール _gt ビジネス通話の分析の Skype"の下にあるビジネス管理センターの Skype の P2P の呼び出しにアクセスして、日付と時刻での呼び出しを関連付けることや、元の (行 ID の呼び出し) CLID のユーザー名または SIP アドレスを指定して検索できます。 
*     
...
> [!NOTE]
> 分の呼び出し、通話や会議のサブスクリプションに含まれていないあたりの唯一の支払を含むようにレポートを実行する場合は、"MCOPSTNPP"の機能を持つレポートをフィルター処理します。 これを行うと、分の呼び出しごとのすべての支払の明細を表示が提供されます。  分のオーディオ会議ごとの支払、"MCOPSTNPP"ではなく"MCOMEETACPEA"でフィルターを適用します。  
***
> [!NOTE]
> "データはありませんいくつかのフィールドにも表示します。 「データがありません」では、フィールドには適用されない呼び出しの種類や機能を意味します。 
***
> [!NOTE]
> Telstra の通話プランの場合は、表示されません、PSTN の使用状況レポートに詳細レコードを呼び出します。 レポートの要件は、Telstra に問い合わせてください。 
***
