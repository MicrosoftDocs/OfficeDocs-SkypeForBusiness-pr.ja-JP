...
ビジネス ユーザーは、Skype を展開する前に、[オンライン ビジネスの Skype の設定](set-up-skype-for-business-online.md)資料では、手順 1 ~ 3 を行っています。 この方法では、ビジネスの Skype 設定、ドメインに、すべてのユーザーは、ライセンスがありが構成した IM と[オンライン ビジネスの Skype のプレゼンスを構成](configure-presence-in-skype-for-business-online.md)、組織の。
  
> [!NOTE]
> ビジネス アプリケーションの Skype をインストールするのには、ユーザーの PC やデバイス上のローカルの管理者に、必要があります。 または、PC やデバイスにアプリケーションをインストールすることができます、そのローカル グループの一部として使用する必要があります。 ユーザーが自分のデバイスにソフトウェアをインストールするのには許可されていない場合は、それらのビジネス アプリケーションの Skype をインストールする必要があります。 
...
ユーザーの代わりに、インターネットからネットワーク上の場所からビジネス アプリケーションの Skype をインストールする場合は、セットアップ ファイルをダウンロードできます。 Office 365 管理センターの [**ユーザーのソフトウェアを手動で配置**] セクションに移動を行う。 **インストール**を選択し、セットアップ .exe ファイルをネットワーク上の場所に保存します。
