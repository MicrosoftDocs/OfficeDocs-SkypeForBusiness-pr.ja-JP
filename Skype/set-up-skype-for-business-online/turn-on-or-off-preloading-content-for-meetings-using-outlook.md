...
> [!NOTE]
> 現在はありませんコマンドレットに使用できるビジネス オンラインの Skype の設定または_MaxContentStorageMB_と_MaxUploadFileMB_のオンラインの値を表示します。 設置型展開ではそれらは。 知っている必要が接続されているコンテンツは、 _MaxUploadFileSizeMB_を超える場合、または_MaxContentStorageMB_の制限に達した場合、会議にそのコンテンツをアップロードできなきます。
