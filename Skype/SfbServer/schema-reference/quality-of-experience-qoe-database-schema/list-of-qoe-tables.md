...
|[ユーザー テーブル](user-0.md) <br/> |ストアのユーザー、会議、および電話の Uri は、オーディオとビデオのセッションで使用されます。  <br/> |
|[エンドポイントのテーブル](endpoint.md) <br/> |オーディオとビデオのセッションに参加しているエンドポイントのコンピューター名の FQDN を格納します。  <br/> |
|[プール テーブル](pool.md) <br/> |メトリクス データが属しているプールの名前を格納します。  <br/> |
|[デバイス テーブル](device.md) <br/> |ストアでは、キャプチャ デバイスと、オーディオとビデオ通話で使用されているデバイスを表示します。  <br/> |
|[早い段階表](devicedriver.md) <br/> |キャプチャ デバイスとオーディオとビデオ通話で使用されているレンダリング デバイス用のドライバーをストアです。  <br/> |
|[会議テーブル](conference.md) <br/> |会議シナリオまたは DialogID の他のシナリオで会議の Uri を格納します。  <br/> |
...
|[ダイアログ テーブル](dialog.md) <br/> |オーディオおよびビデオ ・ セッションのダイアログの ID を格納します。  <br/> |
|[地域テーブル](region.md) <br/> |NC の設定で定義されているネットワーク領域を格納します。  <br/> |
|[UserSite テーブル](usersite.md) <br/> |NC の設定で定義されているネットワーク サイトを格納します。  <br/> |
|[サブネット テーブル](subnet.md) <br/> |NC の設定で定義されているサブネットを格納します。  <br/> |
...
|[サーバーのテーブル](server.md) <br/> |メディアを通過するサーバーの FQDN または IP アドレスを格納します。  <br/> |
...
|[セッション表](session.md) <br/> |オーディオまたはオーディオ/ビデオ ・ セッションに関する全般的な情報を格納します。 セッションは、2 つのエンドポイント間でオーディオまたはビデオの SIP ダイアログとして定義されます。  <br/> |
