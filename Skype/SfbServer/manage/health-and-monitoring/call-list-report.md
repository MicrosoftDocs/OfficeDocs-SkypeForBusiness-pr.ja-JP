...
応答グループの通話リストで応答コードまたは診断 ID の上にマウス ポインターを置いてもこれらの指標の情報は表示されません。 詳細が必要な場合は、応答コードまたは診断 ID をメモし、次に[エラーの上位レポート] でそれらの値を検索[Skype for Business Server。](top-failures-report.md)
