...
|:-----|:-----|:-----|
|[**発信者 PAI**] <br/> |不要  <br/> |呼び出しを開始したユーザーの P-Asserted-Identity。P-Asserted-Identity は、信頼されたネットワーク内でユーザーの証明済み ID を送信するのに使用されます。  <br/> |
|[**発信者 URI**] <br/> |不要  <br/> |呼び出しを開始したユーザーの SIP アドレス。  <br/> |
|[**発信者エンドポイント**] <br/> |不要  <br/> |呼び出しを実行したデバイス。  <br/> |
|[**発信者ユーザー エージェント**] <br/> |不要  <br/> |呼び出しを実行したデバイスで使用されるソフトウェア。  <br/> |
|[**通話の開始**] <br/> |不要  <br/> |呼び出しが最初に開始された日時。  <br/> |
|[**仲介サーバーのバイパス通話**] <br/> |不要  <br/> |呼び出しが仲介サーバーを経由せずに PSTN 音声ゲートウェイまたは適切な IP-PBX に接続されたかどうかを示します。  <br/> |
|[**発信者 OS**] <br/> |不要  <br/> |呼び出し元のコンピューターのオペレーティング システム。  <br/> |
|[**発信者 CPU**] <br/> |不要  <br/> |呼び出しを開始したユーザーのコンピューターに搭載される CPU。  <br/> |
|[**発信者 CPU コア番号**] <br/> |不要  <br/> |呼び出しを開始したユーザーが使用するコンピューターのプロセッサ番号。  <br/> |
|[**発信者 CPU 速度**] <br/> |不要  <br/> |呼び出しを開始したユーザーが使用するコンピューターの CPU のクロック速度。  <br/> |
|[**発信者 CPU 仮想化**] <br/> |不要  <br/> |呼び出しを開始したユーザーが使用するコンピューターでの仮想化 (仮想化されている場合)。  <br/> |
|[**呼び出し先 PAI**] <br/> |不要  <br/> |通話に招待されたユーザーの P-Asserted-Identity。P-Asserted-Identity は、信頼されたネットワーク内でユーザーの証明済み ID を送信するのに使用されます。  <br/> |
|[**呼び出し先 URI**] <br/> |不要  <br/> |呼び出し先ユーザーの SIP アドレス。  <br/> |
|[**呼び出し先エンドポイント**] <br/> |不要  <br/> |呼び出しを受信したデバイス。  <br/> |
|[**呼び出し先ユーザー エージェント**] <br/> |不要  <br/> |呼び出しを受信したデバイスで使用されるソフトウェア。  <br/> |
|**Duration** <br/> |不要  <br/> |通話時間。  <br/> |
|[**メディア バイパス警告フラグ**] <br/> |不要  <br/> |仲介サーバーがバイパスされたときに発行された警告。  <br/> |
|[**呼び出し先 OS**] <br/> |不要  <br/> |呼び出し先ユーザーのコンピューターのオペレーティング システム。  <br/> |
|[**呼び出し先 CPU**] <br/> |不要  <br/> |呼び出し先ユーザーのコンピューターに搭載される CPU。  <br/> |
|[**呼び出し先コア番号**] <br/> |不要  <br/> |呼び出し先ユーザーが使用するコンピューターのプロセッサ番号。  <br/> |
|[**呼び出し先 CPU 速度**] <br/> |不要  <br/> |呼び出し先ユーザーが使用するコンピューターの CPU のクロック速度。  <br/> |
|[**呼び出し先 CPU 仮想化**] <br/> |不要  <br/> |呼び出し先ユーザーが使用するコンピューターでの仮想化 (仮想化されている場合)。  <br/> |
