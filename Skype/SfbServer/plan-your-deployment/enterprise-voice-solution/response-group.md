...
このSkype for Business Server、応答グループの管理には、応答グループ マネージャーと応答グループ管理者の 2 つの管理役割を使用できます。 応答グループ管理者は、任意の応答グループの任意の側面を管理できます。 応答グループマネージャーは、特定の側面のみを管理し、自分が所有する応答グループに対してのみ管理できます。 管理者の役割は、特定の応答グループに対する制限付き責任を、管理者が有効にしているユーザーに委任できるので、管理コストを削減するのに役立エンタープライズ VoIP。 ユーザーには、応答グループ マネージャーと応答グループ管理者の両方を指定できます。
...
- Skype for Business Server コントロール パネル
    ...
    > [!NOTE]
    > 応答グループ管理者は、このツールを使用して応答グループの設定のみを管理できます。 その他Skype for Business Server管理者は使用できません。
...
応答グループ アプリケーションは、サーバーを展開するときに自動的にエンタープライズ VoIP。
...
- **サーバー間通信用のポート 8404**
   ...
   > [!NOTE]
   > これらのポートは既定の設定であり、**Set-CsApplicationServer** コマンドレットを使用して変更することができます。 このコマンドレットの詳細については、「管理シェル」Skype for Business Serverを参照してください。
...
Windows メディア オーディオ ファイル形式では、Windows メディア形式ランタイムが Windows Server 2008 R2 および Windows Server 2008 を実行しているフロント エンド サーバーにインストールされている必要があります。 詳細については、このセクションで前述した「ソフトウェア要件」を参照してください。
...
> [!NOTE]
> 次の表の数値は、すべての応答グループオーディオ ファイルに 16 kHz、モノラル、16 ビットウェーブ (.wav) ファイルを使用すると仮定しています。 メディア オーディオ (.wma) などのWindowsファイル形式を使用する場合は、数値が異なる場合があります。
