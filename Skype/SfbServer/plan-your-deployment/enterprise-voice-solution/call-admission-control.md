...
通話受付管理 (CAC) は、受け入れ可能な品質のリアルタイム セッションを確立するのに十分なネットワーク帯域幅が必要かどうかを判断します。 このSkype for Business Server CAC は、オーディオとビデオのリアルタイム トラフィックのみを制御しますが、データ トラフィックには影響しません。 既定の WAN パスに必要な帯域幅が存在しない場合、CAC はインターネット パスまたは公衆交換電話網 (PSTN) を介して通話のルーティングを試行できます。
...
管理者は CAC ポリシーを定義します。これは、すべてのフロントエンド プールにインストールされている帯域幅ポリシー サービスによって適用されます。 CAC 設定は、ネットワーク内のすべてのフロントエンド Skype for Business Serverに自動的に伝達されます。
...
CAC では、ネットワーク領域ごとにSkype for Business Serverサイトを定義する必要があります。 中央サイトは、そのネットワーク地域内の他のすべてのサイトへの最適なネットワーク接続と最高帯域幅に基づいて選択されます。 前のネットワーク トポロジの例には 3 つのネットワーク地域が示されており、それぞれに CAC の決定を管理する中央サイトがあります。 前の例の適切な関連付けを次の表に示します。
...
- 音声ビデオ会議サービスSkype for Business間の電話会議では、G.722 または Siren が使用されます。

- 公衆交換電話網 (PSTN) への呼び出しは、Skype for Business エンドポイント間で G.711 または RTAudio (8kHz) のどちらかを使用します。
...
G.722、Skype for Business Server会議の既定のコーデックは、G.722.1 および Siren コーデックとは完全に異なります。

Siren コーデックは、次の状況Skype for Business Serverで使用されます。
...
|PSTN 通話 (Skype for Business PSTN ゲートウェイ間、メディア バイパス付き)  <br/> |97 kbps  <br/> |97 kbps  <br/> |161 kbps  <br/> |
|PSTN 通話 (Skype for Business仲介サーバー間、メディア バイパスなし)  <br/> |45 kbps  <br/> |97 kbps  <br/> |161 kbps  <br/> |
