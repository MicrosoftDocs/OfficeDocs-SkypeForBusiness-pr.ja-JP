---
title: Skype for Business Server 2015 でのエンタープライズ VoIP の復元の計画
ms.author: crowe
author: CarolynRowe
manager: serdars
ms.date: 8/17/2015
...
詳細については、これらのフェーズは、展開に関するドキュメントで[、リカバリ性に優れたブランチ アプライアンスまたはサーバーを展開する](http://technet.microsoft.com/library/cb780c14-dc5f-41ba-8092-f20ae905bd16.aspx)を参照してください。
...
なお、ユーザー レベルのダイヤル プランと音声ポリシーを作成してから、ブランチ サイトのユーザーに割り当てることもお勧めします。 詳細についてを参照してください[を作成するビジネス サーバー 2015 の Skype のダイヤル プランを変更するまたは](../../deploy/deploy-enterprise-voice/dial-plans.md)と展開に関するドキュメントでは、[ブランチ ユーザーの VoIP ルーティング ポリシーを作成します](http://technet.microsoft.com/library/10deca9f-f870-4a42-b25d-e4fc53108658.aspx)。
...
組織では、直接内側ダイヤル (DID) 電話番号を個々 のユーザー用に構成されていない場合は、内線番号だけを持つ各ユーザーの回線 URI が構成されている内部ユーザー互いに呼び出し、内線番号だけをダイヤルすることによって。 ただし、内線番号に一致する中央サイトのユーザー、ブランチ サイトのユーザーからの呼び出しに適用できる正規化ルールを構成する必要があります。
