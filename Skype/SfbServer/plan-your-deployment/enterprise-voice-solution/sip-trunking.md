...
多くの場合、音声機能が SIP トランキングを展開する主な動機ではありますが、音声サポートは最初のステップにすぎません。 SIP トランキングでは、VoIP 機能を拡張でき、Skype の豊富なサービスを提供するビジネスのサーバーを有効にできます。 次に例を示します。
...
> [!IMPORTANT]
> フェールオーバーを含め、高可用性のサポートを提供しているかどうかを確認するには、サービス プロバイダーに問い合わせてください。 サポートしている場合、高可用性を設定する手順を確認する必要があります。 各仲介サーバーでは、1 つの IP アドレスと 1 つの SIP トランクを構成する必要があるなど、各仲介サーバーで複数の SIP トランクを構成する必要がありますか。 _gt、セントラル サイトが複数ある場合に、サービス プロバイダーが別のセントラル サイトとの間の接続を有効にする機能を持つかどうかも依頼してください。

> [!NOTE]
> SIP トランキングでは、スタンドアロンの仲介サーバーを展開することを強くお勧めします。 詳細については、「展開」のドキュメントの「[Deploying Mediation Servers and Defining Peers](https://technet.microsoft.com/library/a684f1da-6671-4011-adf6-2db49e2528e2.aspx)」を参照してください。
