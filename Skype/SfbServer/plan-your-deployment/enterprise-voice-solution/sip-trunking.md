...
SIP トランクは、組織とファイアウォールの外側のインターネット テレフォニー サービス プロバイダー (ITSP) との間に SIP 通信リンクを確立する SIP 接続です。 通常、SIP トランクを使用して組織の中央サイトを ITSP に接続します。 ブランチ サイトを ITSP に接続するために SIP トランキングを使用する場合もあります。

SIP トランキングの展開は、組織の通信を簡素化し、リアルタイム通信を最新の機能拡張に備えするための大きな一歩となります。 SIP トランキングの主な利点の 1 つは、組織の接続を中央サイトの公衆交換電話網 (PSTN) に統合できるという利点があります。前任者の時刻分割多重化 (TDM) トランキングとは対照的に、通常は各ブランチ サイトとは別のトランクが必要です。
...
- ブランチ サイトに存続可能な通話接続が必要な場合 (WAN がダウンした場合など)。 この要件は、ブランチ サイトごとに分析する必要があります。一部のブランチでは冗長性とフェールオーバーが必要な場合があります。他のブランチでは必要ない場合があります。

- 回復性は、2 つの中央サイト間で必要です。 SIP トランクが各中央サイトで終了する必要があります。 たとえば、ダブリンとトゥクウィラの中央サイトを持ち、両方とも 1 つのサイトの SIP トランクのみを使用している場合、トランクがダウンした場合、他のサイトのユーザーは PSTN 通話を発信できません。

- ブランチ サイトと中央サイトは、異なる国/地域にあります。 互換性と法律上の理由により、国/地域ごとに少なくとも 1 つの SIP トランクが必要です。 たとえば EU では、通信は現地の中央ポイントで終了することなしに国/地域外にでることはできません。
...
- 通話量が少ない小規模の企業またはブランチ サイトでは、インターネットを介した SIP トランキングが最適な選択になる場合があります。 この接続の種類は、中規模サイズあるいは比較的大規模なサイトには推奨されません。
...
> [!IMPORTANT]
> フェールオーバーを含め、高可用性のサポートを提供しているかどうかを確認するには、サービス プロバイダーに問い合わせてください。 サポートしている場合、高可用性を設定する手順を確認する必要があります。 たとえば、仲介サーバーごとに 1 つの IP アドレスと 1 つの SIP トランクのみを構成する必要がありますか、または各仲介サーバーで複数の SIP トランクを構成する必要がありますか? > 複数の中央サイトがある場合は、サービス プロバイダーが別のセントラル サイトとの間の接続を有効にできるかどうかも確認します。

> [!NOTE]
> SIP トランキングでは、スタンドアロンの仲介サーバーを展開することを強く推奨します。 詳細については、「展開」のドキュメントの「[Deploying Mediation Servers and Defining Peers](/previous-versions/office/lync-server-2013/lync-server-2013-deploying-mediation-servers-and-defining-peers)」を参照してください。
...
- 仲介サーバーと境界ネットワーク内の VPN ルーターの間に仮想 LAN (VLAN) を設定します (DMZ、非武装地帯、スクリーン サブネットとも呼ばれる)。
...
- 仲介サーバーを含む任意の場所に VPN サーバー トラフィックをルーティングするルーティング ルールをブロックします。
