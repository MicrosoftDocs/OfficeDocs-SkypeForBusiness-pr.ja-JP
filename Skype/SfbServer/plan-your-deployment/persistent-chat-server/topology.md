...
たとえば、4 台のサーバーの永続的なチャットのサーバーで配置では、80,000 ユーザーができる場所にサインインして同時に、永続的なチャットを使用して、負荷が均等に分散サーバーあたり 20,000 ユーザーで。 1 台のサーバーが使用できなくなった場合、そのサーバーに接続しているユーザーは、永続的なチャット サーバーへのアクセスをできなくなります。 切断されたユーザーは、利用できなくなったサーバーが復元されるまでは、残りのサーバーに自動的に転送されます。 
