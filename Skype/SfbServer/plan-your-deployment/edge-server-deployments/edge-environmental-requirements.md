...
&sup1;Exchange DNS 負荷分散を使用したユニファイド メッセージング (UM) リモート ユーザー のフェールオーバーには、2013 以降Exchange必要があります。
  
### <a name="ip-address-requirements"></a>IP アドレスの要件

基本的なレベルでは、3 つのサービスに IP アドレスが必要です。Access Edge Service、Web 会議エッジ サービス、および音声ビデオ エッジ サービス。 3 つの IP アドレス (サービスごとに 1 つ) を使用するか、1 つを使用して各サービスを別のポートに置くオプションがあります (一部の詳細については、「ポートと[](edge-environmental-requirements.md#PortFirewallPlan)ファイアウォールの計画」セクションを参照してください)。 単一の統合エッジ環境では、これはかなり多くの場合です。
...
- **ハードウェア負荷分散: ほとんどのシナリオ** で DNS 負荷分散をお勧めします。 もちろん、ハードウェア負荷分散もサポートされますが、特に DNS 負荷分散を超える 1 つのシナリオで必要になります。
...
|**Location**|**Type**|**ポート**|**FQDN または DNS レコード**|**IP アドレスまたは FQDN**|**注**|
|:-----|:-----|:-----|:-----|:-----|:-----|
...
|**Location**|**Type**|**ポート**|**FQDN**|**FQDN ホスト レコード**|**注**|
|:-----|:-----|:-----|:-----|:-----|:-----|
...
|**Location**|**Type**|**ポート**|**FQDN**|**IP アドレスまたは FQDN ホスト レコード**|**注**|
|:-----|:-----|:-----|:-----|:-----|:-----|
...
少なくとも、外部エッジ サーバー インターフェイスに割り当てられた証明書は、パブリック証明機関 (CA) によって提供される必要があります。 特定の CA をお勧めしませんが、お客様が優先する CA が一覧に表示される[](../../../SfbPartnerCertification/certification/services-ssl.md)場合は、CA、ユニファイド コミュニケーションの証明書パートナーの一覧を参照してください。
...
- contoso.com<span></span>: パブリック FQDN
    
- fabrikam.com<span></span>: 2 番目のパブリック FQDN (複数の SIP ドメインがある場合に要求する情報のデモとして追加)
    
- Contoso.net<span></span>: 内部ドメイン
...
|**プロトコル**|**TCP または UDP**|**ポート**|**送信元 IP アドレス**|**接続先 IP アドレス**|**注**|
|:-----|:-----|:-----|:-----|:-----|:-----|
