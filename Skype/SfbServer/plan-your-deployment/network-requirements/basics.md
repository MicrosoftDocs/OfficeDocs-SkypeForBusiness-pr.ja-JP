...
> [!IMPORTANT]
> 既定ドメインに参加していないコンピューターのコンピューター名は、ホスト名としない、完全修飾ドメイン名 (FQDN) です。 トポロジ ビルダーでは、ホスト名ではなく、Fqdn を使用します。 そのため、ドメインに参加していないエッジ サーバーとして展開するコンピューターの名前を DNS サフィックスを構成する必要があります。 **唯一の標準文字を使用**(A-Z、a-z、0-9、およびハイフンを含む) Skype をビジネスのサーバーを実行しているサーバーに Fqdn を割り当てるとします。 Unicode 文字およびアンダースコアは使用しないでください。 外部の DNS およびパブリック Ca では、FQDN の非標準文字はサポートされて多くの場合 (つまり、とき、FQDN 割り当てる必要があります証明書の SN に)。
