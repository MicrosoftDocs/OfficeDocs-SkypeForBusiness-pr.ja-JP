...
10. このカテゴリに属するチャット ルームでファイルのアップロードを制御するには、[**ファイルのアップロードを有効にする**] チェック ボックスをオンまたはオフにします。オンにした場合、このカテゴリのルームでは、ファイルのアップロードを有効または無効にできます。オフにした場合、このカテゴリのルームではファイルのアップロードは許可されません。
     ...
     > [!IMPORTANT]
     > カスタム アプリケーションまたは Office 通信 Server 2007 R2 グループ チャット サーバーまたは Lync Server 2010、グループ チャットを使用する前のグループのチャット クライアントは、ルームにファイルを投稿できるため、サーバーでこの設定が適用されます。 Lync 2013 クライアントには、ファイルのアップロード/ダウンロード機能がないため、永続的なチャット サーバーのチャット ルームでファイルを投稿することはない場合は、純粋な Lync 2013 の展開または Lync 2013 クライアントがある場合は、します。 
