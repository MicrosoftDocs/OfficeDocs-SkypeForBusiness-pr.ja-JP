...
- [**外部 Web サービス**] を構成するには、**HTTP** および **HTTPS** 用の [**リッスン ポート**] を定義します。既定では、それぞれ、TCP ポート 80 および TCP ポート 443 です。また、**HTTP** および **HTTPS** 用の [**公開ポート**] を構成します。既定では、それぞれ、TCP ポート 80 および TCP ポート 443 です。内部 Web サービスの構成およびロード バランサー (ロード バランサー機器および DNS 負荷分散) の使用に基づいて、ポート値を調節して、リッスン ポートおよび公開ポートを定義します。
    ...
    > [!IMPORTANT]
    > 外部 Web サービスおよび定義したリッスン ポートと公開ポートは、外部 クライアントおよび外部デバイス用です。外部クライアントおよび外部デバイスは、外部 Web サービスのリッスン ポートと公開ポートを使用します。これらは、通常、定義済み外部 Web サービスの完全修飾ドメイン名 (FQDN) とともにリバース プロキシによって定義されます。外部ユーザーおよび外部デバイスが利用できるサービスに、外部クライアントがアクセスする際使用する URL (Uniform Resource Locator) アドレスは、外部 Web サービスの FQDN と簡易 URL の関係によって定義されます。簡易 URL に関する詳細については、「[Planning for Simple URLs](/previous-versions/office/lync-server-2013/lync-server-2013-planning-for-simple-urls)」を参照してください。
