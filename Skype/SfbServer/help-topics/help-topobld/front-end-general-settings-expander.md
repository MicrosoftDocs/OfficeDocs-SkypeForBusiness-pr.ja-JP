...
- [**特徴と機能**] で、このプールに併置する追加の役割を定義します。次の設定を構成できます。
  ...
  - [**エンタープライズ VoIP**]。 内部のボイスオーバー IP 通話を、修飾されたハンドセットとデバイス、および Skype for Business クライアントに対して有効にします。 外部通話機能を有効にするには、仲介サーバーを含める必要があります。 詳細については、このトピックの「仲介サーバー」を参照してください。
