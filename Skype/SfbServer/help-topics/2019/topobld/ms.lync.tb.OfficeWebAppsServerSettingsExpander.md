 ...
 **Office Web Apps サーバー検出 url**: Office Web apps サーバーへのクライアントアクセス用の uniform resource LOCATOR (url)、展開用の内部ネットワーク以外のネットワークゾーンにサーバーが配置されている場合は、既定のアドレスを編集することが必要になることがあります。
