...
[**オンライン証明書要求の状態**] ページには、オンライン証明書要求を適切に作成および発行した結果として、重要な情報が表示されます。 このページには、証明書を一意に識別する証明書の拇印が表示されます。 既定では、[証明書の使用状況にこの証明書を割り当Skype for Business Server **] チェック ボックスが** オンになっています。 [完了] **をクリック** すると、証明書要求のSkype for Business Serverに定義した目的で、証明書が自動的に割り当てられます。 既定では、証明書を割り当てる目的は次のとおりです。
...
[**完了**] をクリックすると、オンライン証明書要求プロセスが完了します。 [この証明書を証明書の使用状況に割り当Skype for Business Server **選択** した場合、証明書は自動的に割り当てられます。 このチェック ボックスをオフにした場合は、別の手順で証明書を割り当てる必要があります。 
