...
- 処理能力の計画のためには、次の計算に基づいて、1 つまたは複数の Cloud Connector がオフラインになった場合の負荷への対応能力を検討する必要があります。
  ...
  - **N+1 ボックス。** より大きなバージョンの Cloud Connector の場合、N + 1 のボックス\*では、500の同時呼び出しを 99.8% の可用性でサポートします。

    クラウドコネクタの小型バージョンについては、N + 1 の\*ボックスで50の同時呼び出しを 99.8% の可用性でサポートします。

  - **N+2 ボックス。** クラウドコネクタの大規模のバージョンでは、N + 2 の\*ボックスで500の同時呼び出しが 99.9% の可用性でサポートされています。

    クラウドコネクタの小型バージョンでは、N + 2 のボックスで\*、50の同時呼び出しを 99.9% の可用性でサポートします。
...
- 必要な DNS レコードと SRV レコードをすべて作成した Office 365 テナント。
    ...
    > [!IMPORTANT]
    > テナントを Cloud Connector エディションと統合すると、組織の SIP ドメインとして、既定のドメインサフィックス onmicrosoft.com が使用されます。 > sip を使用することはできません。\<クラウドコネクタ\>エッジアクセスプロキシインターフェイスの名前としてのドメイン名。この DNS レコードは、Office 365 によって使用されているためです。
...
.ini ファイルにおいて、SIP ドメインと同じドメイン空間のゲートウェイに FQDN 名を設定すると、この SIP ドメインの認証ゾーンが境界内の DNS サーバーに作成されます。 エッジサーバーで名前を解決するためにこの DNS サーバーがポイントされている場合、Edge では _sipfederationtls は解決されません。\<yourdomain\> DNS レコード。通話フローに必要です。 この場合、Microsoft は、Edge の外部インターフェイスでインターネット名の参照を解決するために DNS サーバーを用意し、各エッジコンポーネントがホストファイルを使用して、他のクラウドコネクタのコンポーネント名を IP アドレスに解決することをお勧めします。
