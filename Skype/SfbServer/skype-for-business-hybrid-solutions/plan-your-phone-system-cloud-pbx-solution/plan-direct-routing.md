...
# <a name="plan-direct-routing"></a>直接ルーティングを計画します。
  ...
  > [!NOTE]
  > これは、マイクロソフト電話システム直接ルーティングのプレビュー版です。  製品の機能およびドキュメントは、変更されることができます。
...
- [メディア トラフィック: IP アドレスとポートの範囲](#media-traffic-ip-addresses-and-port-ranges)
- [SBCs のサポートされています。](#supported-session-border-controllers-sbcs)

直接ルーティングの構成の詳細については、[直接ルーティングの構成](configure-direct-routing.md)を参照してください。
...
- [は](https://www.audiocodes.com/solutions-products/products/products-for-microsoft-365/direct-routing-sbcs-media-gateways)
- リボン (以前は Sonus)。
...
[直接ルーティングを構成します。](configure-direct-routing.md)
