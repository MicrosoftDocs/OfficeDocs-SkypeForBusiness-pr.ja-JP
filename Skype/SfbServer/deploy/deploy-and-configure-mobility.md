...
2. [**スタート]**、[管理ツールの選択]  **(スタート メニュー** のオプションではない場合は検索が必要な場合があります **)、[DNS]** をクリックして DNS 管理スナップインを開きます。
...
証明書の計画に関する質問がある場合は、「Plan for Mobility for Mobility for Skype for Business Server」[に記載](../plan-your-deployment/mobility.md)されています。 その確認が完了したら、次の手順を実行します。
...
1. 管理シェルと Ocscore がインストールされている任意のSkype for Business Server **CsAdministrator** **役割の** メンバーとしてログオンします。
    
2. 管理シェル **Skype for Business Server起動** します (検索に名前を入力するか、[すべてのプログラム] に移動 **して選択します**)。
...
1. **CsAdministrator** 役割のメンバーであるアカウントを使用して、管理シェルと **Ocscore** がインストールSkype for Business Server **コンピューター** にログオンします。
    
2. 管理シェル **をSkype for Business Serverします**。
