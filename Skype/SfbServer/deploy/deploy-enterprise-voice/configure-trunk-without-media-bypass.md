...
3. [**トランク構成**] ページで、次のいずれかの方法を使用してトランクを構成します。
   ...
   - [**プール トランク**]: [**サービスの選択**] で、このトランク構成が適用されるトランクの名前を選択して [**OK**] をクリックします。 このトランクには、ルートのトランク、またはトポロジ ビルダーで定義されている任意の追加トランクを指定できます。 特定のトランクに対してトランク構成が既に作成されている場合、[**サービスの選択**] にはトランクは表示されませんので注意してください。

     > [!NOTE]
     > トランク構成のスコープは、選択後に変更することはできません。 _gt **[名前**] フィールドは、トランクの構成の関連するサイトまたはサービスの名前があらかじめ入力されて、変更できません。
