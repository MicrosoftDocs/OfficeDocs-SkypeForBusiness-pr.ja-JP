...
6. [**変換ルール**] で、変換される E.164 電話番号の形式のパターンを次のように指定します。
   ...
   - 一致パターンで指定した桁数を表す値。 たとえば、一致するパターンは、^ (翻訳ルールで \d{7})$、$1 は、7 桁のダイヤル番号を表します。
    
   - (省略可能)変換された数値 (たとえば、+1425) に前置する桁数を指定する**数字を追加する**フィールドに値を入力します。
    
    例では、ダイヤルする番号と**変換ルール**のパターンと**一致させるパターン**の contains^(\d{7})$ が含まれています + 1425 E.164 のパターンとして、$1 の電話番号、ルールが +14255550100 に 5550100 を正規化します。
...
6. [**正規表現の入力**] で次のように入力します。
   ...
   - [**変換ルール**] で、変換される E.164 電話番号の形式のパターンを指定します。
    ...
    入力する場合など、^ (**このパターンに一致**することで \d{7})$ と + 1425**翻訳ルール**ルールでは 1 ドルが +14255550100 に 5550100 を正規化します。
