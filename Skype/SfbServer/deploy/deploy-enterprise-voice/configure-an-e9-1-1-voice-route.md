...
E9-1-1 を展開するには、まず緊急通話用のボイス ルートを構成する必要があります。 音声ルートの作成の詳細については、「音声ルートを作成または変更する」を参照[Skype for Business。](create-or-modify-a-voice-route.md) 展開にプライマリ SIP トランクおよびセカンダリ SIP トランクが含まれている場合などは、複数のルートを定義できます。 
  
> [!NOTE]
> E9-1-1 INVITE に位置情報を含めるには、E9-1-1 サービス プロバイダーに接続する SIP トランクを構成して、ゲートウェイを介して緊急通話をルーティングする必要があります。 これを行うには、 **Set-CsTrunkConfiguration コマンドレットの EnablePIDFLOSupport フラグを** True に設定します。 EnablePIDFLOSupport の既定値は False です。 たとえば、 `Set-CsTrunkConfiguration Service:PstnGateway:192.168.0.241 -EnablePIDFLOSupport $true.` 代替の公衆交換電話網 (PSTN) ゲートウェイおよび緊急場所識別番号 (ELIN) ゲートウェイの受信場所を有効にする必要はありません。
