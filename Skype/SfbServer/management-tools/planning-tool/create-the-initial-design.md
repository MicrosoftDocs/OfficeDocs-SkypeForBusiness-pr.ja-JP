...
4. 設計に対して該当する条件を選択して各セクションに回答し、[**次へ**] をクリックして、ウィザードの次のページに進みます。前の各ページを変更するには、[**戻る**] をクリックします。
    ...
    > [!TIP]
    > 各ページには、選択条件の説明および望ましい方法や処理能力計画に基づいた推奨事項が表示されます。 追加の詳細情報を必要とする場合は、Microsoft TechNet web サイトのドキュメントについてはサーバーの 2015 計画のビジネス Skype から詳細な情報を読み取るため**の詳細を表示**をクリックします。 Microsoft TechNet Web サイトにアクセスするには、インターネットに接続する必要があります。
