...
**概要:** 通話品質ダッシュボードを使用する方法について学習します。 品質ダッシュボードの呼び出しは、ユーザーのSkype for Business Server。
...
CQD は、QoE (Quality of Experience) データに基づいてレポートをすばやく作成および整理するための Web ポータルです。 CQD は SSAS キューブを展開して QoE Metrics データベース内のデータを集計し、管理者はレポートを作成および変更したり、リアルタイムで調査を行います。 キューブに直接接続するためにExcelを使用することもできますが、ポータルは QoE データを含む複数のワークフロー用に最適化されています。 データには次の情報が含まれます。
...
トップ レベルでは、"Audio ストリーム月次傾向" レポートには、すべてのオーディオ ストリームの月次傾向が表示されます。 棒グラフのバーの上にマウスを移動すると、棒グラフで表されるデータの詳細なビューが表示されます。 [オーディオ ストリーム 月次傾向] レポートのタイトルをクリックして、[管理された通話と管理されていないオーディオ ストリーム] レポートに移動します。レポートは管理通話とアンマネージ通話の間で分割されます。 管理された呼び出しは、有線接続を使用して企業ファイアウォール内から行われた呼び出しです。 管理されていない呼び出しには、企業ファイアウォールの外部から行われた呼び出しと、Wi-Fi を使用して行われたすべての呼び出しが含まれます。

もう 1 つのトップ レベル レポートは、「ユーザーが報告した通話品質評価ヒストグラム」と呼ばれる。 通話品質評価は、通話のSkype for Businessユーザーが通話の品質を示すために指定した番号です。 評価番号の範囲は 1 ~ 5 で、1 は最悪で、5 が最適です。 ヒストグラムは、1 か月で指定された評価を受け取ったオーディオ通話の数を示します。
...
最初に、Managed ストリームを確認して修復します。 この領域の品質は、コントロール内で 100% にし、修正が最も簡単である必要があります。

### <a name="managed-streams"></a>マネージ ストリーム
