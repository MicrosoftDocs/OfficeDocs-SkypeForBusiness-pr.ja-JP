...
|テスト-CsP2PVideoInteropServerSipTrunkAV (P2PVideoInteropServerSipTrunkAV)  <br/> |ビデオの相互運用機能のサーバーが起動して、ビデオの SIP トランク経由で着信接続を処理できることを確認します。  <br/> |
...
|テスト-CsXmppIM (XmppIM)  <br/> |インスタント メッセージを Extensible Messaging and Presence Protocol (XMPP) ゲートウェイ経由で送信できることを確認します。  <br/> |
...
||**説明**|**サポートされている場所**|
|:-----|:-----|:-----|
|向こう側にあります。  <br/> |内部サーバーを偽装し、認証の問題を回避するには、証明書を使用します。  <br/> ウォッチャー ノードごとに多くのユーザーのパスワードの代わりに、単一の証明書を管理する管理者のために便利です。  <br/> |中企業。  <br/> この方法では、監視ノードが監視対象のプールと同じドメイン内に存在する必要があります。 監視ノードとプールが別のドメインに存在する場合は、この方法ではなく、資格情報認証を使用します。  <br/> |
|Negotiate  <br/> |格納ユーザー名およびパスワードの安全に Windows 資格情報マネージャーで各監視ノードにします。  <br/> このモードは、パスワード管理の手間を必要としますが、エンタープライズの外側に位置する監視ノードの場合には唯一の選択肢となります。 このような外側に位置する監視ノードを認証の際に信頼済みのエンドポイントとして扱うことはできません。  <br/> |社内外です。  <br/> 中企業。  <br/> |
