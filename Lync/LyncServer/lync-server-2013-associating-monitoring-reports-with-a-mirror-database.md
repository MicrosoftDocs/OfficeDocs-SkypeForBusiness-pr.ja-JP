...
ミラーデータベースに自動的にフェールオーバーするように監視レポートを取得するには、監視レポートで使用される2つのデータベースに対してミラーデータベースを "フェールオーバーパートナー" として追加する必要があります (1 つは通話詳細レコードデータ用、もう1つは QoE (Quality of Experience) データ)。 (この手順は、監視レポートをインストールした後に実行する必要があります。)フェールオーバーパートナー情報を追加するには、これら2つのデータベースで使用される接続文字列値を手動で編集します。 これを行うには、次の手順を実行します。
...
2.  Reporting Services のホームページにアクセスした後、[ **LyncServerReports**] をクリックしてから、[ **レポートの \_ コンテンツ**] をクリックします。 これにより、Lync Server 監視レポートの [ **レポートの \_ コンテンツ** ] ページに移動します。
...
7.  [ **Cdrdb** ] ページで、[ **レポート \_ コンテンツ** ] リンクをクリックします。 **Qmsdb**データソースをクリックし、qmsdb データベースの接続文字列を編集します。 以下に例を示します。
