...
---
# <a name="replication-cmdlets-in-lync-server-2013"></a>Lync Server 2013 のレプリケーションのコマンドレット
...
レプリケーションのコマンドレットを使用すると、Lync Server レプリケーションの監視と管理の両方を行うことができます。 これらのコマンドレットを使用して、レプリケーション設定を構成できます。レプリケーションの進行状況を監視するにはサーバーでレプリケーションを手動で強制的に実行します。

<div>

## <a name="replication-cmdlets"></a>Replication Cmdlets

以下に、レプリケーションの管理に直接関連するコマンドレットの一覧を示します。
