...
パブリックインスタントメッセージング接続の証明書を構成するには、最初に、他の SIP フェデレーションタイプまたは標準エッジサーバー証明書とは異なるものがあることに注意してください。ただし、America Online (AOL) には、固有の証明書の構成が必要です。 通常のサーバー拡張キー使用法 (EKU) に加えて、America Online では証明書または証明書 (エッジプールの場合) にもクライアント EKU を含める必要があります。 クライアント EKU は証明書に追加され、エッジサーバーに割り当てられている外部公開証明書の一部です。
