...
2.  ホストボイスメール用のユーザーアカウントを構成するには、Set-CsUser コマンドレットを実行します。 たとえば、以下を実行します。
    ...
    上述の例では、次のパラメーターが設定されます。
      ...
      - **Identity** により、変更するユーザー アカウントを指定しています。 Identity 値は、以下の形式のいずれかで指定できます。
          ...
          - ユーザーの Active Directory ドメイン サービス表示名 (例: Ken Myer)。 Identity 値として表示名を使用する場合は、アスタリスク (\*) ワイルドカード文字を使用できます。 たとえば、"\* smith" という id は、"smith" という文字列値で終わる表示名を持つすべてのユーザーを返します。
