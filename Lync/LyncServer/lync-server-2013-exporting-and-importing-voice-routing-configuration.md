...
> [!IMPORTANT]  
> [<STRONG>音声ルーティング</STRONG>] グループ内のいずれかのページで、設定に対して未確定の変更を行った場合、これらの変更はエクスポートされた音声構成ファイル (.vcfg) に保存されます。 これにより、変更を公開する前に、複数の Lync Server コントロールパネルセッション中に音声ルーティング構成を変更することができます。
