...
Lync Server 2013 は、ライセンス付きのパブリックインスタントメッセージング (IM) 接続プロバイダーの使用と、lync Server が Lync 2013 クライアントを使用して構成された XMPP ドメインパートナーにアクセスできるようにする特別な種類のフェデレーションを実装するための拡張メッセージングおよびプレゼンスプロトコル (XMPP) の使用をサポートしています。
...
Windows Live ユーザーとの通信の場合、Lync Server 2013 は、ピアツーピア IM および音声通話およびビデオ通話をサポートしています。 AOL および Yahoo との通信で \! は、Lync Server 2013 はピアツーピア IM をサポートしています。 別のライセンスが必要な場合があります。
