...
description: 'Lync Server 2013: 通話受付管理のコマンドレット。'
ms.reviewer: ''
ms.author: v-lanac
author: lanachin
f1.keywords:
...
ms.locfileid: "48569203"
---
# <a name="call-admission-control-cmdlets-in-lync-server-2013"></a>Lync Server 2013 での通話受付管理のコマンドレット
...
通話受付管理 (CAC) は、許容できる品質のリアルタイムの音声またはビデオセッションを確立するのに十分なネットワーク帯域幅があるかどうかを判断します。 CAC を管理するには、ネットワーク、サイト、およびサブネットの制限と設定を構成し、それらの間の相互作用を行います。

<div>

## <a name="call-admission-control-cmdlets"></a>Call Admission Control Cmdlets

次のコマンドレットを使用して、Lync Server 管理シェルから CAC を管理します。
