...
ms:mtpsurl: https://technet.microsoft.com/ja-jp/library/Dn767949(v=OCS.15)
ms:contentKeyID: 62486271
ms.date: 05/19/2016
...
To run this check using actual user accounts, you must create two Lync Server 管理シェル credentials objects (objects that contain the account name and password) for each account. You must then include those credentials objects and the SIP addresses of the two accounts when you call Test-CsWebAppAnonymous:
...
Configuration problems with Office Web Apps サーバー can also cause Test-CsWebAppAnonymous to fail; that will often be the case if you receive the following diagnosis:

The HTTP request is unauthorized with client authentication scheme 'Ntlm'. The authentication header received from the server was 'Negotiate,NTLM'.

For more information on diagnosing and resolving Office Web Apps サーバー problems see the blog post [Office Web Apps Server 2013 - machines are always reported as Unhealthy](http://www.wictorwilen.se/office-web-apps-server-2013---machines-are-always-reported-as-unhealthy).
