...
10. [証明書要求ファイル] ページで、要求を保存するファイルの完全なパスとファイル名を入力します (たとえば、c:\\cert\_外部\_edge. .cer)。
...
16. [サブジェクト代替名 (San) の SIP ドメイン設定] ページで、[ドメイン] チェックボックスをオンにして sip を追加します。\<microsoft.rtc.management.xds.sipdomain\>エントリをサブジェクトの別名一覧に入力します。
...
23. XMPP フェデレーションの DNS を構成するには、次の SRV レコードを外部 DNS\_: xmpp サーバーに追加します。\_tcp。\<ドメイン名\> SRV レコードは、エッジサーバーのアクセスエッジ FQDN に解決され、ポート値は5269になります。 さらに、アクセスエッジサーバーの IP アドレスをポイントする "A" ホストレコード (たとえば、xmpp.contoso.com) を構成します。
