...
![ABSConfig ツール。](images/JJ945604.6fb63a70-7b63-4b8b-b7d1-82fe9aa2028f(OCS.15).jpg "ABSConfig ツール。")
...
![帯域幅使用率のファイルディレクトリ](images/JJ945604.d66daeac-1669-45e3-932d-3f6782840c2a(OCS.15).jpg "帯域幅使用率のファイルディレクトリ")
...
![帯域幅使用率の概要でのデータ収集 y](images/JJ945604.0019cb2c-7c01-4dc9-ac90-ac47c47d1bfd(OCS.15).jpg "帯域幅使用率の概要でのデータ収集 y")

入力の検証が正常に行われると、以下のメッセージが表示されます。

![帯域幅 Utili で収集された通知をログに記録します]。(images/JJ945604.eda91da8-3285-4eab-8ccb-c6d89c8cc221(OCS.15).jpg "帯域幅 Utili で収集された通知をログに記録します")。
...
1.  BandwidthUtilizationAnalyzer.xlsm が自動的に起動したら、以下の図に示す [**Refresh**] をクリックします。
    ...
    ![BandwidthUtilizationAnalyzer](images/JJ945604.c4e675b9-1671-400e-a712-6db82d731b39(OCS.15).jpg "BandwidthUtilizationAnalyzer")
...
4.  グラフィカル プロットが自動的に生成されます。バックグラウンドで作業中のポインターが表示されなくなると、使用できます。
    ...
    ![レポートビューでのフィルターの適用](images/JJ945604.1416468e-e3ab-478e-b569-e42ba9c27a17(OCS.15).jpg "レポートビューでのフィルターの適用")
...
![BandwidthUtilizationAnalyzer で名前によるフィルター処理](images/JJ945604.002b7c8e-f0da-48ce-9e1a-5c34d2cab063(OCS.15).jpg "BandwidthUtilizationAnalyzer で名前によるフィルター処理")

[**Exceeded limit**] でフィルター処理します。 フィルターを適用するには、[**True**] を選びます。

![制限を超えてフィルター処理しています。](images/JJ945604.5946c95e-76ce-46ca-8f3e-a79be1e5c527(OCS.15).jpg "制限を超えてフィルター処理しています。")

[**Critical levels **] でフィルター処理します。 フィルターを適用するには、[**TRUE **] を選びます。

![重要レベル別にフィルター処理します。](images/JJ945604.60771a52-d8ba-4cb9-a02d-d6c888cb5505(OCS.15).jpg "重要レベル別にフィルター処理します。")

[**Under-utilized **] でフィルター処理します。 フィルターを適用するには、[**TRUE **] を選びます。

![[使用率] でフィルター処理します。](images/JJ945604.95a2bf01-5aba-4927-af47-1ad3c459d791(OCS.15).jpg "[使用率] でフィルター処理します。")
...
![領域ごとのフィルター処理。](images/JJ945604.5de4cec4-6c09-48bb-98c7-b56f7bdb3d5a(OCS.15).jpg "領域ごとのフィルター処理。")
...
![呼出しの範囲内の範囲をオービットで指定します。](images/JJ945604.9ede64cb-29d9-4782-a34b-b76c42fbdcad(OCS.15).jpg "呼出しの範囲内の範囲をオービットで指定します。")

現在パークされている通話: –n パラメーターによって、このプールで現在使用されているすべてのオービットが次のように一覧表示されます。

![現在保留中の通話が保留中です。](images/JJ945604.07a7eec4-7999-4c92-93f0-95525b244b4c(OCS.15).jpg "現在保留中の通話が保留中です。")

使用されていないオービットの数: –f パラメーターによって、プールで現在使用されていないオービットの数が次のように一覧表示されます。

![Orbits の無料通話。](images/JJ945604.ecc1d621-0ca0-4ecf-a579-08b41c6f08ed(OCS.15).jpg "Orbits の無料通話。")

最近の保留中の通話: \<–\> r n パラメーター \<は\> 、次に示すように、最後の保留中の通話を一覧表示します。

![最近保留になった通話の着信の一覧です。](images/JJ945604.1c5eb27d-faa1-491b-b4aa-b484255c3353(OCS.15).jpg "最近保留になった通話の着信の一覧です。")

次に示すように、軌道\<予約\>をテストします。-t n パラメーターテストでは、データベース内でオービットを予約しています。

![呼び出しの軌道で軌道予約をテストします。](images/JJ945604.84c9b69e-7af0-4224-8711-a43a28f08691(OCS.15).jpg "呼び出しの軌道で軌道予約をテストします。")
...
DBAnalyze は、管理者が Lync Server 2013 データベースに関する分析レポートを収集するのに役立つコマンドラインツールです。 DBAnalyze には、診断モード、ユーザー データ モード、会議モード、MCU モード、ディスク断片化モードが用意されています。
  ...
  - **診断モード**   テーブル (レコード数、断片化、データサイズ、インデックスサイズ)、データとログファイルのサイズ、最後のバックアップ時間、Microsoft を実行しているサーバー間の連絡先配布に関する情報を含むレポートを作成します。Office Communications Server、アクセス許可の平均数、連絡先、コンテナー、サブスクリプション、文書、ユーザーごとのエンドポイント、不適切なホームユーザー、ルーティングできないユーザー、ユーザーごとに開催された会議の平均数、スケジュール会議、アクティブな会議、データベースのバージョン。
  ...
  - **会議モード**   では、特定の会議の詳細データが報告されます。これには、会議のすべてのスケジュール、出席者リスト、会議で許可されているメディアの種類の一覧、アクティブな mcu (multipoint control units)、アクティブな状態が含まれます。参加者リスト、および各参加者のシグナリング状態。
...
![Dbanalyze .exe の権限テーブル]。(images/JJ945604.b8931e9e-834e-4dec-8a84-2fc47d1613e9(OCS.15).jpg "Dbanalyze .exe の権限テーブル")。
...
![サンプルストレージサーバーイベントログイベント。](images/JJ945604.3a903ef7-ea8a-4606-8229-a3e32f13af3a(OCS.15).jpg "サンプルストレージサーバーイベントログイベント。")
...
MSTurnPing ツールでは、Microsoft Lync Server 2013 通信ソフトウェアの管理者が、オーディオ/ビデオエッジと音声/ビデオ認証サービスを実行しているサーバーの状態と、帯域幅ポリシーを実行しているサーバーの状態を確認することができます。トポロジ内のサービス。

<div>

## <a name="description"></a>説明

MSTurnPing ツールを使用すると、Lync Server 2013 通信ソフトウェアの管理者は、オーディオ/ビデオエッジと音声/ビデオ認証サービスを実行しているサーバーの状態と、の帯域幅ポリシーサービスを実行しているサーバーの状態を確認することができます。トポロジ.
...
![ネットワーク構成をダウンロードしています。](images/JJ945604.8d126d3f-2545-4f13-a244-974f09614982(OCS.15).jpg "ネットワーク構成をダウンロードしています。")
...
![帯域幅ポリシーサーバーログファイルを開く。](images/JJ945604.3e503e92-aacb-4921-a8d2-23f860fe2df6(OCS.15).jpg "帯域幅ポリシーサーバーログファイルを開く。")
...
CAC ネットワークトポロジ図を JPG または BMP 形式で保存して保存します。 Lync Server 2013 管理者は、**ネットワーク構成ダイアグラムを画像として保存**することで、cac ネットワークトポロジ構成をグラフィカルな形式 (JPG と bmp ファイル形式) で保存することができます。オプションが表示されます。
...
![ツリービューでネットワーク構成データを表示する。](images/JJ945604.4d924ac9-fd96-430f-b211-ee35b7ef9a23(OCS.15).jpg "ツリービューでネットワーク構成データを表示する。")
...
![ネットワーク用のカスタムコネクタを定義します。](images/JJ945604.26262c75-4342-41c3-bc98-1793aa6a7713(OCS.15).jpg "ネットワーク用のカスタムコネクタを定義します。")
...
![応答グループエージェントのライブツール。](images/JJ945604.63cb0374-a6ef-4a59-b60e-bec86a880d09(OCS.15).jpg "応答グループエージェントのライブツール。")
...
![[応答グループエージェント Live オプション] ダイアログボックス。](images/JJ945604.3cc15e29-8699-45ab-90c3-e1565fa6ebf6(OCS.15).jpg "[応答グループエージェント Live オプション] ダイアログボックス。")
...
![通話を発信したり、IM を送信したりすること。](images/JJ945604.009cebe0-5a93-4745-89c3-8a16c7c13009(OCS.15).jpg "通話を発信したり、IM を送信したりすること。")

下の図は、Response Group Agent Live に表示されるキュー内の現在の通話数と全着信通話内での最長待機時間を示しています。

![キュー情報を表示します。](images/JJ945604.131d7f79-b7ed-41f5-a9da-ffc556e31037(OCS.15).jpg "キュー情報を表示します。")
...
Sysprep.inf スクリプトを実行する前に、必須ファイルを Windows Server 2008 オペレーティングシステムコンピューター上のローカルフォルダー (例 **: D:\\Setup)** にコピーする必要があります。 このフォルダーには、Lync Server 2013 ファイルのコピー (具体的には**** setup.exe) も含める必要があります。 必須ファイルは、次の場所からダウンロードできます。
...
Unassigned Number Announcements Migration スクリプトは、次の 3 つのシナリオで使用できます。
  ...
  - **構成設定を新しいバージョンの Lync Server に移行する:** Contoso は、Lync Server 2013 への移行プロセス中であり、移行プロセスの一環として、lync server の管理者が、アナウンスメントアプリケーションによって提供されている、割り当てられていない番号の構成を Lync Server 2010 展開から新しい Lync Server 2013 の展開。 設定を移動するには、Lync Server の管理者が、割り当てられていない番号のお知らせ移行ツールを使用します。
...
![ムーブグループのアナウンスの設定パラメーター。](images/JJ945604.7ab66ad3-d0db-4d77-8b93-ebccf0cb0663(OCS.15).jpg "ムーブグループのアナウンスの設定パラメーター。")
