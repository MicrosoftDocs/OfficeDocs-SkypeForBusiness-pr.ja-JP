...
2. 早期導入者プログラムの目標と結果を共有するキックオフコールをホストします。
3. チームワーク チャンピオン チームのフィードバック チャネルと、使用できるその他の内部ソーシャル ネットワーク ツールへのアクセスを確認します。 
4. 対象となる情報を収集する定期的なアンケートを作成して完了を検討します。

## <a name="gather-feedback"></a>フィードバックの収集

プロジェクトのフィードバックを収集Microsoft 365またはOffice 365コンポーネントを使用します。
  
![判断ポイントを表すアイコン](media/teams-adoption-decision-icon.png) 決定: 組織が既に Yammer ネットワークに投資している場合は、早期導入者プログラムのフィードバック フォーラムとして使用できます。 それ以外の場合、早期導入プログラムのメンバー数が 5,000 人未満の場合は、Teams でチームを作成してフィードバックを収集し、トレーニングを提供します。
...
![次の手順を表すアイコン 次 ](media/teams-adoption-next-icon.png) : オンボード [サポート](teams-adoption-onboard-support.md)。
