...
- **利用できる言語は組織の場所によって決まります** 。 たとえば、組織が米国内にある場合は、既定の言語を英語またはスペイン語に設定できます。 組織がカナダにある場合は、英語とフランス語から選べます。 サポートされている言語の一覧については、Teams と Skype for Businessを参照してください。
...
- **個々のユーザーのボイスメール応答メッセージとボイスメール メッセージの言語を変更する。** ユーザーの優先言語を変更できます。この言語を使用すると、ボイスメール応答メッセージの言語と、ユーザーのメールボックスに送信されたボイスメール メッセージOutlookできます。 詳細については、「言語または言語の言語と地域の設定を設定する[Microsoft 365」をOffice 365。](/office365/troubleshoot/access-management/set-language-and-region)
...
2. [組織Microsoft 365 管理センター プロファイル]**を設定設定**  >    >  **選択します**。
...
- [オンプレミス電話システム PSTN Microsoft 365接続Office 365を使用して、オンプレミスの PSTN 接続を使用して、Skype for Business Server](/skypeforbusiness/skype-for-business-hybrid-solutions/plan-your-phone-system-cloud-pbx-solution/plan-phone-system-with-on-premises-pstn-connectivity)
