...
発信者番号は、次の 2 つのユーザー向けの識別可能な情報で構成されます。

- 電話番号 (通常は CLID または通話回線 ID と呼ばれます)。 これは、発信者の ID として表示される公衆交換電話番号 (PSTN) です。

- 発信側の名前 (通常は CNAM と呼ばれます)。 
  
発信者番号機能は、PSTN 接続オプションに関係なく、すべての電話システム ユーザーが使用できます。
...
- 次の代替電話番号を指定できます。
  ...
  - 通話プランの電話番号インベントリで、サービスとフリーダイヤル番号として分類される電話番号。 通常、自動応答または通話キュー Teamsに割り当てられます。

  - Teams自動応答または通話キューによって使用されるリソース アカウントに割り当てられる、ダイレクト ルーティングを介したオンプレミスの電話番号。 
...
エンド ユーザーは、通話を **設定 >** して発信者番号を匿名に設定し、[**発信者番号]** で **[自分の電話番号とプロファイル情報を非表示にする**] を選択します。
...
- リソース アカウントの電話番号の置換の使用は、Teams ユーザーに対してのみ機能します。 サービス電話番号の置き換えは、Skype for Business Online ユーザーと Teams ユーザーの両方で機能します。

- 発信者名は、発信者 ID が LineUri(サービスまたはリソース アカウントの電話番号)に置き換わり、発信者がTeams ユーザーの場合にのみ送信されます。
...
電話システム着信外部電話番号が発信者番号として表示されます。 番号がAzure ADまたは個人の連絡先のユーザーまたは連絡先に関連付けられている場合、Skype for Business クライアントとTeams クライアントは、その情報に基づいて発信者番号を表示します。 電話番号がAzure ADまたは個人の連絡先にない場合は、電話番号が使用可能な場合は、電話会社が指定した表示名が表示されます。
