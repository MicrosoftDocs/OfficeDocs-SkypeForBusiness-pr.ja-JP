...
- **通話キュー** レポートの **通話結果** ビジュアルでは、多数の **_不明_** な呼び出しが報告される場合があります。 これは、サポートが修正に取り組んでいる通話分類の問題が原因です。  これは呼び出し分類の問題のみで、これらの呼び出しはシステムによって正常に処理されました。

- 呼び出しに応答する最初の自動応答または呼び出しキュー内の呼び出しと呼び出し元のアクションのみが報告されます。  チェーンされた自動応答 (1 つの自動応答が別の自動応答に転送される場合) またはチェーンされた呼び出しキュー (1 つの呼び出しキューが別の呼び出しキューに転送される場合) の呼び出しと呼び出し元のアクションは報告されません。 

- 通話キューと自動応答は、呼び出しキューまたは自動応答名ではなく、リソース アカウントの ID によって表示されます。  自動応答または通話キューのすべてのトラフィックを表示するには、自動応答または通話キューに割り当てられているすべてのリソース アカウントを選択する必要があります。

- 通話キューと自動応答データは個人データと見なされ、データプライバシー保持ポリシーの対象であるため、ダッシュボードで使用できるのは 28 日間の履歴のみです。
