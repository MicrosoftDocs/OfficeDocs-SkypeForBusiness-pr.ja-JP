...
---
# <a name="powershell-script-sample---reset-the-autostart-setting-in-teams"></a><span data-ttu-id="cca0a-103">PowerShell スクリプトのサンプル - Teams の自動開始設定をリセットする</span><span class="sxs-lookup"><span data-stu-id="cca0a-103">PowerShell script sample - Reset the autostart setting in Teams</span></span>

<span data-ttu-id="cca0a-104">このスクリプトを使用して、Teams の自動開始設定をユーザー単位でリセットします。</span><span class="sxs-lookup"><span data-stu-id="cca0a-104">Use this script to reset the Teams autostart setting on a per-user basis.</span></span> <span data-ttu-id="cca0a-105">これには、ユーザーまたは Teams アプリによって設定された値が含まれます。</span><span class="sxs-lookup"><span data-stu-id="cca0a-105">This includes any values set by the user or the Teams app.</span></span> <span data-ttu-id="cca0a-106">既定では、インストール後にユーザーが自分のコンピューターにログインすると、Teams が自動的に開始されます。</span><span class="sxs-lookup"><span data-stu-id="cca0a-106">By default, Teams automatically starts when a user logs in to their computer after it's installed.</span></span>

<span data-ttu-id="cca0a-107">既に Teams を展開し、インストール後に [Microsoft Teams](../msi-deployment.md#use-group-policy-recommended) が自動的に開始されるのを防ぐグループ ポリシー設定を設定して Teams の自動開始を無効にする場合は、最初にグループ ポリシー設定を必要な値に設定してから、このスクリプトを実行する必要があります。</span><span class="sxs-lookup"><span data-stu-id="cca0a-107">If you've already deployed Teams and want to set the [Prevent Microsoft Teams from starting automatically after installation Group Policy setting](../msi-deployment.md#use-group-policy-recommended) to disable Teams autostart, you'll need to first set the Group Policy setting to the value you want, and then run this script.</span></span>

<span data-ttu-id="cca0a-108">Teams がユーザーに対して開始された後は、グループ ポリシーを使用して自動開始設定を無効にすることはできません。</span><span class="sxs-lookup"><span data-stu-id="cca0a-108">After Teams is started for a user, the autostart settings can't be disabled by using Group Policy.</span></span>

## <a name="sample-script"></a><span data-ttu-id="cca0a-109">サンプル スクリプト</span><span class="sxs-lookup"><span data-stu-id="cca0a-109">Sample script</span></span>
...
## <a name="related-topics"></a><span data-ttu-id="cca0a-110">関連項目</span><span class="sxs-lookup"><span data-stu-id="cca0a-110">Related topics</span></span>

- [<span data-ttu-id="cca0a-111">MSI を使用した Teams のインストール</span><span class="sxs-lookup"><span data-stu-id="cca0a-111">Install Teams using MSI</span></span>](../msi-deployment.md)
- [<span data-ttu-id="cca0a-112">エンタープライズ向け Microsoft 365 アプリで Teams を展開する</span><span class="sxs-lookup"><span data-stu-id="cca0a-112">Deploy Teams with Microsoft 365 Apps for enterprise</span></span>](/deployoffice/teams-install)