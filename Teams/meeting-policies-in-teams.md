...
[ **今すぐ会議** を許可する] は、ユーザーごとのポリシーで、会議が開始される前に適用されます。 この設定は、ユーザーが Teams のチャネルで予定外の会議を開始できるかどうかを制御します。 この設定を有効にした場合、ユーザーは [ **会議** ] ボタンを選択して、計画されていない会議を開始するか、またはチャネルで会議をスケジュールすることができます。 既定値は True です。
...
このオプションを無効にした場合、ユーザーはスケジュールを設定できません。 Outlook で新しい会議を作成するときの Teams 会議 たとえば、Windows 上の Outlook では、[**新しい Teams 会議**] オプションはリボンに表示されません。
...
- [チャネル返信の作成] ボックスの [**会議の予約**] ボタン。
  ![[会議の予約] ボタンの [返信の作成] ボックス](media/schedule-meeting-disabled-in-chat2.png)
  
- チャネルヘッダーの [**会議の予約**] ボタン。
  ![チャネルヘッダーの [会議の予約] ボタン](media/schedule-now-in-header.png)

チャネルの予定表で、次の操作を行います。

- チャネル予定表ヘッダーの [**新しいイベントの追加**] ボタンは無効になります。
  ![無効になっているチャンネルの予定表ヘッダーのボタン](media/add-new-event-disabled.png)
...
これはユーザーごとのポリシーであり、会議が始まる前に適用されます。 この設定は、ユーザーが予定外のプライベート会議を開始できるかどうかを制御します。  既定では、この設定はオンになっています。
...
> [!Note]
> この機能はまだ開発段階です。 画面共有は、参加者単位のポリシーです。
