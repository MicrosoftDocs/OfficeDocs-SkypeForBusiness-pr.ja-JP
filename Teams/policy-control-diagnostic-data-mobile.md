...
- **composeSendSmartReply** - スマート返信アイテムがクリックされます。
...
- **editSmartReply** - スマート返信アイテムが編集されます。
...
- **Expected: botClickCardAction - Android: showCard - iOS: missing** - カード ボタンをタップします。カードは主要なプラットフォーム コンストラクトで、その用途とパターンを測定することは、プラットフォームの用途を理解し、クライアント側の潜在的な問題を探し出すのに必要となります。
...
- **tabActionOpenInTeams** - 開く用途。これは、タブが既定で [Teams で開く] に設定できるかどうかを理解する上で重要です。
...
- **teamChannelChanged** - ユーザーがプラン リストからプランを選択して移動するとトリガーされます。Aria ではなく appInsights にのみ送信されます。
...
- **tenantSwitch** - テナントの切り替え。MTMA の機能の成功指標 (複数のテナントと複数のアカウント) 機能は、予防的に問題を特定して解決し、スムーズな切り替えエクスペリエンスを提供するのに役立ちます。
...
- **updateTaskState** - タスクの状態が更新されたことを確認します。アクション。
