...
- PSTN 通話を別の Teams ユーザーに転送するには、転送を開始するユーザーと同じネットワーク サイトにターゲット ユーザーのエンドポイントが位置している必要があります。 他のシナリオでは、転送は許可されません。 
- 別のTeamsユーザーを PSTN に転送するには、最初の呼び出し元と同じネットワーク サイトにある Location-Based ルーティングが有効なゲートウェイを介して通話を転送する必要があります。 他のシナリオでは、転送は許可されません。
...
|---------|---------|
|ユーザーの DID が割り当てられているのと同じサイトで、Location-Based ルーティングが有効になっている (Site1)      |ユーザーの音声ルーティング ポリシーに基づいて、Site1 で Location-Based ルーティング (GW1) が有効になっているゲートウェイ経由でルーティングされた通話         |
...
|ユーザーの DID が割り当てられているサイトとは異なります。サイトがルーティングに対してLocation-Based (Site3)    | Site3 のエンドポイントにルーティングされない呼び出し        |
...
|異なるネットワーク サイト、Location-Based ルーティングが有効なサイト (User3)|許可されない|許可されない|
...
|異なるネットワーク サイト、Location-Based ルーティングが有効なサイト (User3)    |通話転送は、User3 の音声ルーティング ポリシーLocation-Based基づいて、Site1 でルーティングが有効な Gateway1 経由でのみルーティングできます。         |コールフォワードは、User3 の音声ルーティング ポリシーLocation-Based基づいて、Site1 でルーティングが有効な Gateway1 経由でのみルーティングできます。         |
...
次の表は、User1 Location-Based PSTN 通話に対して異なるユーザーに対して同時呼び出しを許可するかどうかを示しています。
...
|ネットワーク ルーティング (User4) Location-Based異なるネットワーク サイトが有効になっていない    |通話は、User4 の音声ルーティング ポリシーLocation-Based基づいて、Site1 のルーティング ゲートウェイ 1 経由でのみルーティングできます。         |
...
- 委任者に代わって Location-Basedルーティングが有効な代理人からの発信呼び出しの場合は、同じ規則が適用されます。 通話ルーティングは、代理人の通話承認ポリシー、音声ルーティング ポリシー、および場所に基づいて行います。 詳細については、「ユーザーが[PSTN Teams発信通話を発信する」を参照してください](#teams-user-places-an-outbound-call-to-the-pstn)。 
...
### <a name="changes-from-an-on-premises-location-based-routing-deployment"></a>オンプレミスのルーティング デプロイLocation-Based変更
...
「ルーティングの [ネットワーク設定の構成」Location-Based進む](location-based-routing-configure-network-settings.md)。
