...
|2021 年 7 月 7 日よりロール アウト開始 <br> *(完了)* 　&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|**すべてのお客様 (エンタープライズ、教育、GCC)**<br> OneDrive と SharePoint に記録され、会議中にライブで文字起こしされた Teams 会議の場合は、Microsoft Search で検索して、トランスクリプトに基づいて会議記録ファイルを検索できます。 |
...
現在、録画ビデオ ファイルのクローズド キャプションは、会議のトランスクリプトTeamsリンクされています。 このリンクは、ほとんどの場合、ファイルの有効期間中は残りますが、たとえば、ビデオ ファイルが同じ OneDrive または SharePoint サイト内にコピーされた場合に、新しい録画ビデオ ファイルでキャプションが使用できない場合など、破損する可能性があります。

今後の変更は、ここでおよびメッセージ センターの通知で明確化され、60 日未満の記録ファイルに、移行期間中に会議のトランスクリプトがキャプションとして表示されます。
