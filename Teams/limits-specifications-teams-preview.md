...
|機能     | 上限 |
|------------|---------------|
|会議に参加できるユーザー数 (チャットして電話をかけることができる)  |300. **表示のみ** では、最大 2 万人の聴取のみの参加者が会議に参加できます。開催者は、高度な通信アドオン SKU のライセンスを持っています。<sup>1</sup> [!INCLUDE [template](includes/preview-feature.md)] <br><br>**注:** 政府機関用の Teams (GCC、GCC High、DoD) の場合、制限は 250 のままです。 政府機関のクラウド制限が 250 から 300 に増加し、会議のオーバーフローがサポートされたら、この記事を更新します。   |
...
<sup>1</sup> 表示のみは既定でオンです。 PowerShell を使用して、会議のオーバーフローを無効にすることができます。 
```powershell
Set-CsTeamsMeetingPolicy -Identity Global -StreamingAttendeeMode Disabled
Set-CsTeamsMeetingPolicy -Identity Global -StreamingAttendeeMode Enabled
```
表示のみの参加者は、会議に表示専用の容量がない場合、または設定されたロビー ポリシーまたはオプションに基づいてロビーをバイパスする権限がない場合、会議に参加できません。 表示のみの参加者は、ネイティブ PPT 共有ファイルを表示できません。
...
>
> お客様が急速に変化するコミュニケーションのニーズに対応できるように、Microsoft 365 ライブ イベントは、2020 年 10 月 1 日まで、Teams でホストされるライブ イベントの既定の上限を一時的に引き上げます。 次の上限が適用されるようになります。
>
