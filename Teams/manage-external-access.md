...
Teams 管理センターは、組織レベルで外部アクセスを制御します。 ユーザー レベルで外部アクセスを制御する場合は、PowerShell を使用する必要があります。 詳細については、以下の「[PowerShell の使用](#using-powershell)」を参照してください。
...
|特定のドメインとのフェデレーションの有効化|`-AllowedDomains`|N/A|
|特定のドメインとのフェデレーションの無効化|`-Blocked Domains`|N/A|
