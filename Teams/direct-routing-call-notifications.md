...
呼び出しは、正常な呼び出しとして Microsoft 電話の呼び出し詳細レコードに登録されていません。 呼び出しは、Final SIP Code "487"、Final Microsoft サブコード "540200"、最後の SIP コード フレーズ "他の場所で完了しました" を含む "試行" として登録されます。  (通話の詳細レコードを表示するには、管理ポータルTeams分析とレポート、利用状況レポートに移動し、[PSTN の使用状況] を選択します)。
