...
|**問題のタイトル**|**動作 / 症状**|**既知の回避策**|**発見日**|
|:-----|:-----|:-----|:-----|
|不在着信を管理するために、デスクトップアプリでボイスメールにアクセスすることができない|ユーザーを Teams 専用モードに設定している場合は、Exchange メールボックスが内部専用になり、Teams デスクトップアプリで **[設定]** > **[通話]** にアクセスした場合、 通話をボイスメールに転送するオプションを選ぶことができません（不在着信、またはすべての通話）。 いずれかのオプションが既に設定されている場合、[設定] の [通話] タブを開くと、ボイスメールへの転送が無効になります。|Teams モバイルアプリを使用して、メールの転送を設定してください。|2019 年 5 月 27 日|
