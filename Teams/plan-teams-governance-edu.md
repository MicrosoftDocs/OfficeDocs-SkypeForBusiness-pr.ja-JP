...
> [!IMPORTANT]
> Microsoft 365 グループの名前付けポリシーを使用するには、1 つ以上の Microsoft 365 グループのメンバーである一意のユーザーごとに、Azure Active Directory Premium P1 ライセンスまたは Azure AD Basic EDU ライセンスが必要です。
...
> [!Note]
> 別のシステムからの入力 (School Data Sync など) を使用してチームが自動的に作成される場合は、入力データが構成した名前付けポリシーに準拠していることを確認します。そうしない場合、チームの作成は失敗します。
...
私たちの経験では、ユーザー教育は責任ある Teams の使用を確実にするのに役立ちます。 チームの作成が匿名ではないとユーザーが理解するとすぐに、チームを不注意で作成した場合の影響を理解し、ツールの誤用を好む傾向があります。

チームを作成できるユーザーを制御する必要がある場合は [、「Microsoft 365](https://support.office.com/article/manage-who-can-create-office-365-groups-4c46c8cb-17d0-44b5-9776-005fced8e618)グループを作成できるユーザーを管理する」を参照してください。
...
学校のセメスターまたは四半期が終了した場合に Teams データを処理する方法を最初に考え、それを削除するか、コースを完了した後でも学生が利用できる状態を維持するかについて考えすることをお勧めします。 学校の予定表は常に念頭に置いておき、設定したポリシーが祝日と競合しないので、ご安心ください。 戦略の実装には次のツールを使用できます。
...
-   特定の Microsoft 365 グループとチームのゲスト アクセスのオンとオフを切り替え、ゲストを招待できる (招待できない) チームを制御します。
-   監査ログを使用して、招待されたゲストに送信された警告を確認します。

詳細については [、「Microsoft 365 グループのゲスト アクセス」を参照してください](https://support.office.com/article/Guest-access-in-Office-365-Groups-bfc7a840-868f-4fd6-a390-f347bf51aff6#PickTab=Manage)。
