...
> [!NOTE]
> Government Cloud Community (GCC) 環境では、コネクタは既定で無効になっています。 有効にする必要がある場合は、ConnectorsEnabled または ConnectorsEnabledForTeams パラメーターを SetOrganizationConfig コマンドレットを使用$true に設定します。 PowerShell を使用して、Exchange Online[する必要があります](/powershell/exchange/connect-to-exchange-online-powershell?view=exchange-ps)。
