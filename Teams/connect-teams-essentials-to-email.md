...
description: Google Workspace など、Microsoft Teams Essentials (AAD Identity) を既存のメール システムに接続する方法について説明します
...
接続すると、スケジュールされた会議に対する応答や、メールボックスと会議で共同作業を行う招待をMicrosoft Teams。 また、Google Workspace のようなサードパーティの会議ソフトウェアを使用して、Teamsから受信した会議を表示したり、操作したりすることもできます。
...
Teams Essentials (AAD) が提供する必要があるすべての機能を利用するには、Microsoft Teams と Exchange Online の間の接続をオンプレミスの Exchange と構成するハイブリッドアプローチを使用します。
...
オンプレミスの Microsoft Teams 環境に Exchange 会議室をデプロイするには、「Microsoft Teams Rooms with [Exchange on-premises - Microsoft Teams |Microsoft Docs](rooms/with-exchange-on-premises.md)
...
ユーザーがアラームや通知Microsoft 365通知を受け取るのを許可するために、Microsoft 365生成されたメールはすべて Google Teams Workspace に転送されます。 ユーザー ID (ユーザーのプライマリ メールなど) を複製できます。 シングル サインオンも可能ですが、必須ではありません。 ユーザーは、サードパーティの予定表Teams予定表から会議に参加Teamsがあります。 別Teams機能は期待通り動作します。

:::image type="content" source="media/essentials-googleworkspace.png" alt-text="EXO と Gmail の間のメール フローの図を示す画像":::

これらの例では、power [Exchange Online Shell V2](/powershell/exchange/exchange-online-powershell-v2&preserve-view=true)モジュール Connectの一部である[Connect-ExchangeOnline](/powershell/module/exchange/connect-exchangeonline?view=exchange-ps&preserve-view=true) PowerShell コマンドレットに依存しています。 Connect-ExchangeOnline を実行するときにエラーが発生する場合は[、EXO V2](/powershell/exchange/exchange-online-powershell-v2?view=exchange-ps&preserve-view=true)モジュールをインストールする を使用してモジュールをインストールするための推奨手順に従ってください。 資格情報Connect-ExchangeOnline求めるメッセージが表示されたら、必ずテナント管理者アカウントを使用してください。
...
13. **既存のドメインを既定** に設定するために表示されるウィンドウで、既定として設定を確認します。
    ...
    ドメインを Microsoft 365 に追加する方法の詳細については、「ドメインをドメインに追加する」に記載[されている手順にMicrosoft 365。](https://support.office.com/article/add-a-domain-to-office-365-6383f56d-3d09-4dcb-9b41-b5f5a5efd611?ui=en-US&rs=en-US&ad=US)
...
3. [基本 **の設定] ウィンドウで、** 基本的なユーザー情報を入力し、[次へ] を **選択します**。
    ...
    - **ドメイン:** ユーザーのアカウントのドメインを選択します。 たとえば、ユーザーのユーザー名が Jakob で、ドメインが contoso.com の場合、ユーザーは jakob@contoso.com を使用してサインインします。
...
2. グループのルートを追加Office 365
    ...
    **オンプレミス/オンプレミスのメールにメールが送信された場合に ATTR35 応答コードを解決Exchangeスマート ホストメソッドExchange Online。**
...
***Google ワークスペースにサブドメインを追加して、ユーザーからメールを受信Microsoft 365。***

  次に、メールボックスをサブドメインにMicrosoft 365ルールを作成します。 Google Workspace で使用するサブドメインを選択して、Microsoft 365からメールを受信します (g.contosolandscaping2.m365master.com など)。
...
3. テナントから送信されたメールを一意に識別する電子メール ヘッダー Microsoft 365します。 (例: X-MS-Exchange-CrossTenant-id: 92f60fc7-eab3-403b-9d7d-9d683bf0a4b5)
...
15. [ **上記の式が一致する** 場合] で、[メッセージの変更] > **フィールド** を実行し、[迷惑メール] の下のこのメッセージに対して [スパム フィルターをバイパス **する** ] を **オンにしてください**。
...
5. 新 **しいコネクタ ウィンドウの**[接続] で、[接続]**をOffice 365**
...
9. [ルーティング] セクションで、適切なスマート メール ホストを入力し (aspmx.l.google.com など)、[次へ] を **+** 続行 **します**。
...
*予定表Exchange Onlineに直接アクセスする方法を構成する*
...
1. 「メールボックスのメールボックスを有効または[無効Outlook on the web」](/exchange/recipients-in-exchange-online/manage-user-mailboxes/enable-or-disable-outlook-web-app)の手順に従ってExchange OnlineメールボックスのOutlook on the webを無効にします。
...
1. この規則は、管理センターまたは **PowerShell Exchangeで****構成できます**。 次の PowerShell の例 **を使用して** 、ルールを作成できます。
...
1. ナビゲーションで [ドメイン] を設定 >し、[ドメインの追加] を選択します。
...
6. SPF TXT レコードを構成してレコードを含Microsoft 365
...
1. Gmail を構成して、Exchange Onlineメールをシステムに取り込む
