...
## <a name="can-i-port-out-my-numbers-from-teams-to-a-different-phone-service-provider-or-carrier"></a>チームの番号を別の電話サービスプロバイダーまたは携帯電話会社に発信することはできますか?

チームから別の通信事業者に電話番号を移植するには、新しい配送業者に依頼を送信する必要があります。 また、Microsoft Teams 管理センターで移植用 PIN を設定する必要があります。

移植 pin を定義するには、Microsoft Teams 管理センターの左のナビゲーションで、[**音声**  >  **電話番号**] に移動し、ページの右上隅の [**移植 PIN の管理**] を選び、10桁の pin を入力します。

新しい通信事業者から移植要求によって連絡があった場合は、定義した PIN を入力するように求められます。
