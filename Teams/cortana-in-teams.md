...
> [!Note]
> Cortana の音声アシスタンスは、Microsoft Teams iOS および Android モバイル アプリ、Windows の Microsoft Teams 会議室、および Microsoft Teams のディスプレイでサポートされています。これは、米国内のユーザーに対してのみです。 現在、GCC、GCC-High、DoD、EDU テナントでは利用できません。 その他の言語と地域への拡張は、将来のリリースの一環として行います。
...
この例では、既存のポリシーを名前 &#8220;EmployeeCortanaPolicy&#8221; で更新し、プッシュ ボタン呼び出しのみを使用して Microsoft Teams で Cortana 音声アシスタンスを有効にしています。 ユーザーは Teams で Cortana のマイク ボタンを選択して Cortana を起動できます。 単語を覚ます (&#8220;Cortana&#8221; または cortana &#8220;&#8221;) 呼び出しは無効になります。  
