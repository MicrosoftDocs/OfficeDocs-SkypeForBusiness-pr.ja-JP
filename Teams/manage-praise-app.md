...
[**ユーザー設定のバッジ**] トグルを [オン] に切り替えて、[**カスタムバッジの作成**] を選択します。 そこから、サイドパネルでカスタムバッジを設計できます。
...
3. バッジ画像をアップロードします。 許可されているファイルの種類はです。.PNG. ファイルのサイズは25kb 未満でなければなりません。
...
6. [**適用**] を選びます。 これで、新しいバッジがカスタムバッジテーブルに表示されるようになります。
...
**画像を選択するときは、バッジの寸法を念頭に置いておきます。** 最高の品質を得るには、216x216 ピクセルの画像ファイルをアップロードすることをお勧めします。 これらの寸法に合わせて画像を拡大または変形しないようにします。
