...
- 呼び出しルーティング - *First In、First Out* (FIFO) 順に - エージェントへ

- キュー オーバーフローとタイムアウトの処理オプション

この記事の手順に従う前に、[自動応答と通話キューのTeamsプラン](plan-auto-attendant-call-queue.md)に関する記事を読み[、作業の開始手順](plan-auto-attendant-call-queue.md#getting-started)に従っていることを確認してください。
...
Teams デスクトップ ユーザーと標準通話キューの場合は、呼び出しキューのメンバーの呼び出し元 ID を呼び出しキューのサービス番号または適切な自動応答に設定することを検討してください。 詳細については、「[Microsoft Teamsでの呼び出し元 ID ポリシーの管理](caller-id-policies.md)」を参照してください。
...
個々のユーザーまたはグループをキューに追加する場合は、[ **ユーザーとグループの選択**] を選択します。 
...
エージェントのTeams アカウントは TeamsOnly モードに設定する必要があります。 要件を満たしていないエージェントは、通話ルーティングリストに含まれません。 エージェントが互換性のあるクライアントを使用している場合は、通話キューの電話会議モードを有効にすることをお勧めします。
...
|機能                          |Teams デスクトップ<sup>1</sup> |Teams Web | Teams Mobile<sup>2</sup> |Lync |IP 電話 | 標準呼び出しキュー |チャネル ベースの呼び出しキュー | コメント |
|:--------------------------------|:------------------------:|:--------:|:--------------:|:---:|:--------:|:--------------------:|:------------------------:|:--------|
|**エージェント ルーティング方法**        |                          |          |                |     |          |                      |                          |   |
...
注:
1. Microsoft Teams Windows クライアント、Microsoft Teams Mac クライアント、仮想化デスクトップ インフラストラクチャにMicrosoft Teams。
...
7. [ユーザー 設定 ポータル] ページをhttps://aka.ms/vmsettings使用します。
...
呼び出しキュー内の呼び出しエージェントでは、次のクライアントがサポートされています。
  ...
  - Microsoft Teams Android アプリ
    ...
    > [!NOTE]
    > ダイレクト ルーティング番号が割り当てられた通話キューでは、クライアント、Lync クライアント、またはエージェントとしてのSkype for Business IP Phone Skype for Businessはサポートされません。 Teams クライアントは [TeamsOnly の共存モード](/microsoftteams/setting-your-coexistence-and-upgrade-settings)でのみサポートされます。
