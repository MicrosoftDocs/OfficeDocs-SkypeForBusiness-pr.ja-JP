...
エージェント のメンバーシップを制御するために Teams チャネルを使用していない場合は、通話キューのメンバーの発信者番号を、通話キューのサービス番号または適切な自動応答に直接設定してください。 詳細については[、「Microsoft Teams](caller-id-policies.md)で発信者番号ポリシーを管理する」を参照してください。
...
> [!NOTE]
> お客様は、お客様の音楽またはオーディオ ファイルを Microsoft Teams サービスで使用するために必要なすべての権利とアクセス許可を個別に消去および保護する責任を負います。 音楽、サウンド エフェクト、オーディオ、ブランド、名前、およびすべての関連する権利保有者のオーディオ ファイル内のその他のコンテンツに含まれる知的財産、その他の権利が含まれる場合があります。このコンテンツには、アーティスト、アクター、パフォーマー、音楽家、曲者、レコード ラベル、音楽パブリッシャー、ユニオン、ギルド、権利管理組織、音楽著作権、サウンド エフェクト、オーディオ、その他の知的財産権を所有、制御、またはライセンスを与えるその他の当事者が含まれます。
...
次のクライアントは、呼び出しキューに対して Teamsチャネルを使用する場合にサポートされます。 
...
> 
> プレゼンス ベースのルーティングSkype for Business、クライアントを使用するエージェントは、通話ルーティングの一覧に含まれません。 エージェントを使用するエージェントSkype for Business、プレゼンス ベースの通話ルーティングを有効にすることはできません。
