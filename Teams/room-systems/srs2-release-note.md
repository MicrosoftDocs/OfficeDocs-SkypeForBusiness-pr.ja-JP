...
- 部屋のシステムデバイスで Bluetooth ビーコンが有効になっていない場合、[室内] 表示が [キャスト] アイコンを削除します。
...
- TLS 1.2 クライアント側のサポート。 オンプレミスのお客様の場合は、Microsoft Teams 用の TLS 1.2 で communciation を有効にするには、Skype for Business Server 2015 Cummulative Update 9 (CU9) または Skype for Server 2019 Cummulative Update 1 (CU1) が必要です。
...
- ローカライズされた Eula (具体的なノルウェー語) での書式設定の問題を修正しました。 EULA OOBE セットアップウィンドウを超えても昇格できません
...
- アプリが起動しない場合のために、通常は SR デバイスで Crestron アプリボタンを押すことによって、アプリが起動しない修正プログラムを適用します。 4.0.19.0 のインストール後、Microsoft Teams 会議アプリの再起動が必要になります。 

### <a name="40180-08272018"></a>4.0.18.0 (08/27/2018)
この更新プログラムで導入されたもの:

- チームモードでの "問題の報告" 機能の改善 (Skype for Business モードで "フィードバックを送信する" と同じ)
- SIP 通話のために Teams から Skype for Business モードにフォールバックする機能を有効にする
...
- 自動チェックにより、システムが最新のプロビジョニングスクリプトを実行していることを確認します。
...
- アプリケーションを更新してテレメトリを改善しました。

### <a name="31990-03142018"></a>3.1.99.0 (03/14/2018)

この更新プログラムで導入されたもの:

- 断続的な会議の参加に関する問題が発生する可能性がある問題が修正されました。
- デバイスの "ハング" エクスペリエンスを引き起こすという問題が修正されました。
...
- 最適化により、IT 担当者は、Windows 10 バージョン1709年1月の更新プログラム以降を使って、it プロフェッショナル向けのイメージを作成することができます。  
...
- ルームにアクティビティがない場合にディスプレイ (本体と背面) がスリープモードに切り替わらない問題を解決します。
...
- アビリティー (組み込みのテーマとカスタムテーマの設定機能)
- パブリックビルドに関するフィードバックを提供する機能
- 会議への参加に関する信頼性の強化されたテレメトリ
- 追加の OMS レポート
- IT 管理者がデバイスをリモートで構成する機能  <!--  - Front-of-Room UX shows room details pre-meeting U2  -->
