...
必要なマイクロソフトのチームまたは Skype のビジネスおよび Exchange アカウントが既に作成してテストしました[マイクロソフト チーム ルームの展開](room-systems-v2.md)で説明したような場合のみ、この手順を実行する必要があります。 ハードウェアとソフトウェアが[マイクロソフト チームの会議室の要件](requirements.md)」に記載する必要があります。 このトピックには次のセクションが含まれます。
...
> [!NOTE]
> マイクロソフト チームの会議室はビジネス環境をデバイスのアカウントが正しく設定されて[マイクロソフト チーム ルームの展開](room-systems-v2.md)で説明したように正しく構成されているマイクロソフトのチームまたは Skype でのみ動作します。
...
マイクロソフト チームの会議室のコンソールは、接続先のサーバーで使用する証明書を信頼する必要があります。 O365 の場合、これらのサーバーはパブリックの証明機関を使用し、これらは Windows 10 で自動的に信頼されるため、この処理は自動的に行われます。 証明機関がプライベートの場合は Active Directory および Windows の証明機関と設置型展開の方法のいくつかのマイクロソフト チームの会議室のコンソールに証明書を追加できます。
...
2. 管理者モードでコンソールを配置する ([管理者モードとデバイスの管理](room-systems-v2-operations.md#AdminMode)を参照してください)。
...
マイクロソフト チームの会議室のコンソールは、ドメインに参加できます。 マイクロソフト チームの会議室のコンソールは、多くのワークステーションのポリシーは、マイクロソフト チームの会議室では互換性がないため PC ワークステーションから別の OU に配置してください。 一般的な例は、パスワードの強制ポリシー マイクロソフト チームの会議室が自動的に起動できなくなります。 GPO の設定の管理方法については、[マイクロソフト チームのルームの管理](room-systems-v2-operations.md)を参照してください。
  
### <a name="to-join-microsoft-teams-rooms-to-a-domain"></a>マイクロソフト チームの会議室をドメインに参加させる

1. では、管理コンソールに記号 ([管理者モードとデバイスの管理](room-systems-v2-operations.md#AdminMode)を参照してください) を考慮します。
