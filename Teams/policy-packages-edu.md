...
- 同じページにある [会議の参加者とゲスト] セクションで、次のようにします:
...
学生に対し、**[チャネルで "今すぐ会議" を許可する]**、**[チャネルミーティングのスケジュールを許可する]**、**[プライベート会議のスケジュールを許可する]**、**[プライベート会議で "今すぐ会議"]** をオフにすることで、学生が主催者として会議をスケジュールしないようにするだけでなく、次のような教育の安全対策も提供しています:

- 学生が教師の前に会議に参加しようとすると、Teams アプリの最新バージョンでは会議に参加することはできません。

- 会議の作成はすべてのユーザーとすべてのライセンスに適用されますが、上記の会議参加ブロックの安全対策は、ユーザーのライセンスの種類に基づいて、Teams 内の教育機関のお客様にのみ適用されます。

ここに、会議の作成に関する各ポリシーのロジックを説明した表を示します。

| 会議の作成ポリシー | 会議を作成する | 参加者がいない会議を開始する | 参加時にロビーをバイパスする | 会議を終了する |
| --- | --- | --- | --- | --- |
| **オン (例: 教師)** | はい | はい | [会議オプション](https://go.microsoft.com/fwlink/?linkid=2093366) によって決定 | はい、開催者として
| **オフ (例: 学生)** | いいえ | いいえ\*\* | [会議オプション](https://go.microsoft.com/fwlink/?linkid=2093366) によって決定 | いいえ

> [!NOTE]
> \*\* これは、EDU ライセンス付与済みのユーザーにのみ適用され、会議、チャネル会議、インスタントの会議、インスタントのチャネル会議に適用されます。

(チャネルからスケジュールされていない会議、またはチャネルで今すぐ会議において) **[会議でチャットを許可する]** ポリシーを [無効] に変更し、学生がその間会議のスケジュールを設定できないようにし、教師にこのポリシーを有効にすると、学生は教師が会議に参加する前も、会議後も、チャットすることはできません。 会議の前、会議中、会議後もチャットの履歴を表示できます。 たとえば、会議が記録された場合は、教師からのメッセージまたは会議の記録リンクを表示できます。

学生と教師の両方に **[会議でチャットを許可する]** ポリシーを無効にしている場合、会議のチャット ウィンドウでチャットすることはできません。 上記の、会議のチャット制限の安全対策は、ユーザーのライセンスの種類に基づいて、Teams 内の教育機関のお客様にのみ適用されます。

ここに、会議でのチャットの許可について説明した表を示します。

| 「会議でチャットを許可する」のポリシー | チャット履歴をいつでも表示する | 会議中にメッセージを投稿する | 会議の開始前や開始後にメッセージを投稿する |
| --- | --- | --- | --- | 
| **すべての人に対してオン** | はい | はい | はい |
| **すべての人に対してオフ** | 該当なし | 該当なし | 該当なし |
| **教師に対してオン、学生に対してオフ** | 教師: はい<br>生徒: はい | 教師: はい<br>生徒: はい | 教師: はい<br>生徒: いいえ\*\* | 

> [!NOTE]
> \*\* これは、EDU ライセンス付与済みのユーザーにのみ適用され、会議とインスタントの会議に適用されます。 チャネル会議やインスタントのチャネル会議には適用されません。
...
> [!NOTE]
> このトピックの詳細については、「[クラスチームで学生のコメントをミュートする](https://support.office.com/article/Mute-student-comments-in-a-class-team-a378de16-ffc0-420c-b08d-e17ec08e7c17)」をご確認ください。
...
学生の保護に関する詳細については、「[遠隔教育で Teams 会議を利用している間、学生の安全を確保する](https://support.office.com/article/keeping-students-safe-while-using-meetings-in-teams-for-distance-learning-f00fa399-0473-4d31-ab72-644c137e11c8)」をご確認ください。
