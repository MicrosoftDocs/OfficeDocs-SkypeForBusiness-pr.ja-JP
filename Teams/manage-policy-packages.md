...
|学歴 (リモート学習を使用した第1学校教師)    |リモート学習を使用するときに、学生の安全性と共同作業を最大限に活用するために、プライマリ教師に適用する一連のポリシーを作成します。      |
|教育機関 (リモート学習を使った第1学校の学生)    |リモート学習を使用するときに、学生の安全性と共同作業を最大限に活用するために、主要な学生に適用される一連のポリシーを作成します。      |
...
|中小規模企業のユーザー (ビジネス用ボイスのない) |ビジネス音声機能を使わずに、中小規模のビジネスユーザーに適用される一連のポリシーとポリシー設定を作成します。|
...
バッチには最大2万ユーザーを含めることができます。 ユーザーは、オブジェクト Id、UPN、SIP アドレス、またはメールアドレスで指定できます。 詳細については、「[ユーザーのバッチにポリシーパッケージを割り当てる](assign-policies.md#assign-a-policy-package-to-a-batch-of-users)」を参照してください。
