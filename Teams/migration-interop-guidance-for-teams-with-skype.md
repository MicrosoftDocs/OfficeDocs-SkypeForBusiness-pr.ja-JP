...
|---|---|---|---|
|諸島|デバイスです。<sup>1</sup>に着陸する場合、着信 VOIP を呼び出すし、受信者を連携させると、モードの諸島を除くと発信者、同じクライアントでの着陸のチャット|両方|エンド ・ ユーザーは、呼び出しと、クライアントでは、チャットを開始できるし、クライアントの会議をスケジュールすることができます。|
...
TeamsUpgradePolicy は、受信フェデレーション チャットと通話のルーティングを制御します。 フェデレーション ルーティングの動作は、同じテナントの scnearios の場合と同じを除く島モードです。  受信者は、島のモードでは。 
