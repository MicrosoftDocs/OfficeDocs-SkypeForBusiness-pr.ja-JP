...
管理者の場合は、次の診断ツールを使用して、ユーザーが直接ルーティング用に正しく構成されていることを検証できます。

1. 以下 **の [テストの** 実行] を選択します。診断は、Microsoft 365 管理されます。 

   > [!div class="nextstepaction"]
   > [テストの実行: 直接ルーティング](https://aka.ms/TeamsDirectRoutingDiag)

2. [診断の実行] ウィンドウで、[ユーザー名] フィールドまたは [電子メール] フィールドにテストするユーザーの電子メールを入力し、[テストの実行]**を選択します**。

3. テストでは、テナント、ユーザー、またはポリシーの構成に対処して、ユーザーが Microsoft Teams のダイレクト ルーティング用に適切に構成されていることを検証するための最適な次の手順が返されます。
...
降格とは、SBC が最初に試みようとしないという意味です。 たとえば、優先順位が等しい sbc1.contoso.com と sbc2.contoso.com を指定します。  
...
1 つのルート内の 2 つ (または複数) の SBC が正常で等しいと見なされた場合、SPC 間で呼び出しを分散Fisher-Yatesシャッフルが適用されます。
