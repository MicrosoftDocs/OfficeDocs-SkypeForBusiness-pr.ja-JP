...
- Dynamics [](/power-automate/get-started-approvals)365 Power Automate、Office 365、または Dynamics 365 のライセンス。

- ユーザーが新しい承認テンプレートを設定するには、Microsoft Forms のライセンスが必要です。

承認アプリを使用するには、Power Automate のライセンスが必要です。アカウントは、最初の承認割り当て時にターゲット環境の承認ユーザー ロールに自動的に追加されます。
...
承認テンプレートは、内部でのみ内部的に使用される準拠ストレージ プラットフォームであるStorage(SDS) に格納されます。 組織スコープのテンプレートは SDS の "テナント シャード" に格納され、チームスコープテンプレートは SDS の "グループ シャード" に格納されます。 つまり、組織スコープのテンプレートは、テナントとチーム スコープのテンプレートの同じ有効期間を共有し、チームの同じ有効期間を共有します。 そのため、チームを完全に削除すると、関連するテンプレートが削除されます。
...
> [!Note]
> 管理者がチームを削除した場合は、Azure Active Directory (AAD) ポータル内でチームを復元して、すべての関連データを復元できます。 1 か月後、または管理者がごみ箱内でこのチームを削除すると、関連するデータはすべて失われる可能性があります。
...
承認アプリは既定で利用可能です。 Teams 管理センターでアプリを無効にできます。
  ...
  5. 切り替えを選択して、組織のアプリを無効にします。
     ...
     ![は、承認アプリの詳細を表示します。](media/approvals-details.png)
...
1. 管理センター Teamsの **[アプリの** 管理] で、[承認] アプリ **を** 選択し、[アプリ] を **設定。**
