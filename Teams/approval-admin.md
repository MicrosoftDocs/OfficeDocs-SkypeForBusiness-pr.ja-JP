...
承認テンプレートは、Microsoft 内部でのみ内部的に使用Storage準拠しているストレージ プラットフォームである、2013 年 3 月 1 日に保存されます。 組織スコープのテンプレートは SDS の "テナント シャード" に格納され、チームスコープのテンプレートは SDS の "グループ シャード" に格納されます。 つまり、組織スコープのテンプレートは、テナントとチーム スコープのテンプレートの同じ有効期間を共有し、チームの同じ有効期間を共有します。 そのため、チームを完全に削除すると、関連するテンプレートが削除されます。
