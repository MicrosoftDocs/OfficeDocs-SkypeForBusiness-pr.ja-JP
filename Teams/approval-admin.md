...
承認アプリは、Teams 内の構造化された承認と構造化されていない承認の両方に対して、監査、コンプライアンス、説明責任、およびワークフローを簡単にする方法を提供します。 
 ...
 ![承認アプリが表示されます。](media/approvals-selection.png)

ユーザーは承認アプリをピン留めしてメニュー バーに保存できます。

 ![は、pin オプションを含む承認アプリを表示します。](media/approvalApp-pin.png)
...
承認アプリをデプロイするには、次の項目に対するアクセス許可が必要です。
...
- Dynamics 365 [Power Automate、Office 365、](/power-automate/get-started-approvals)または Dynamics 365 のライセンス。

- ユーザーが新しい承認テンプレートを設定するには、Microsoft Forms のライセンスが必要です。

承認アプリを使用するには、Power Automate のライセンスが必要です。最初の承認割り当て時に、ターゲット環境の承認ユーザー ロールにアカウントが自動的に追加されます。
...
>[!Note]
>Microsoft Forms サイトでフォーム テンプレートを削除すると、承認テンプレートが破損し、ユーザーは要求を開始できます。 Microsoft Forms で削除された承認テンプレートを開こうとすると、"CDB TableNotFound" というエラーが表示されます。
...
- 管理者が組織全体のテンプレートを初めて作成すると、グローバル管理者やチームのサービス管理者を含む、テナントのすべての管理者用に新しい Teams チームが自動的に作成されます。 これらの管理者はチームの所有者として追加され、組織のテンプレートを共同管理できます。 チームの作成後に組織に新しい管理者を手動でチーム所有者として追加して、組織全体のテンプレートを管理する権限を同じに設定する必要があります。

> [!Note]
> 管理者がチームを削除した場合、Azure Active Directory (AAD) ポータル内でチームを復元し、関連するデータを復元できます。 1 か月後、または管理者がごみ箱内でこのチームを削除すると、関連するデータはすべて失われる可能性があります。
...
承認アプリは既定で利用可能です。 Teams 管理センターでアプリを無効にできます。
  ...
  3. 承認アプリを検索します。
     ...
     ![[アプリの管理] が強調表示Teams管理>ナビゲーションが表示されます。](media/manage-approval-apps.png)

  4. [承認] を選択します。

  5. 切り替えを選択して、組織のアプリを無効にします。

     ![は、承認アプリの詳細を表示します。](media/approvals-details-new.png)
...
承認電子署名機能の場合、サード パーティの署名パートナーは既定で Teams 承認アプリに表示されます。 特定の電子署名プロバイダーを無効にするには、管理センターのアプリ設定Teamsアクセスします。

1. 管理センター Teamsの [アプリの管理]**で**、[承認] アプリ **を** 選択し、[認証]**設定。**
