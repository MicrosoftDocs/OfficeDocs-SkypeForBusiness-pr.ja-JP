...
オーディオ、ビデオ、または会議を展開している場合は、その機能の環境を最適化するための追加の手順を実行できます。 この内容は、高品質な Microsoft Teams の展開の実現と保守管理のために必須となる要件の概要を提供するものです。 最初のパイロットまたは実稼働展開の前に、サービスの管理と品質を計画して展開を成功させることができます。
...
## <a name="key-technical-components-that-affect-user-experience"></a>ユーザーエクスペリエンスに影響を与える主要な技術コンポーネント

ユーザーエクスペリエンスに影響を与える主要な技術コンポーネントは、このセクションで確認されます。 主要コンポーネントを再確認する前に、組織のビジネス目標を達成するためのユーザー エクスペリエンスについて、およびその重要性について理解することが必要不可欠です。 ユーザー エクスペリエンスの定義について、最初に確認しましょう。

### <a name="user-experience-defined"></a>ユーザー エクスペリエンスの定義

ビジネス目標は、Microsoft Teams を展開するときや、ユーザーがチームの主要な共同作業とコミュニケーションソリューションとして採用するときに実現できます。 品質は、良好なユーザーエクスペリエンスを実現するのに役立ちます。これには、使用方法と導入のキー属性が含まれます。 ユーザー、個人、チームを delights する高品質のサービスを提供することによって、お客様の信頼を得ることができ、ビジネスの利益を促進するサービスの新しい革新的な方法を見つけることができます。

その中心には、Teams に対するユーザーのエクスペリエンス、つまりサービスに対する人間の感情や姿勢があります。 ユーザー エクスペリエンスの向上につながる要素は何でしょうか? これには、ユーザーは、チームを使ってチームを使用する方法と、それらを日常的なワークフローに組み込むことで、どこにいても、優れた通話品質を実現し、信頼性の高い接続を実現することができます。 ユーザーエクスペリエンスは、非常に広範な性質を備えています。この記事では、組織で制御できる技術要素にのみ焦点を当てます。 ユーザーの準備に関する追加情報については[、「Teams の組織を準備](https://aka.ms/SkypeToTeams-UserReadiness)する」を参照してください。
...
ユーザーに最適なエクスペリエンスを提供するには、次の重要な側面について考慮する必要があります。エンドポイントは、チームのハードウェア要件とソフトウェア要件を満たしているか。 Wi-Fi ネットワークをサポートするようにエンドポイントを構成して最適化しているかどうか。 音声通話の発着信を行うためにどのデバイスを使用するか。 これらのデバイスが Teams のために最適化されているか。
...
[プロジェクトチーム](upgrade-enlist-stakeholders.md)を集めるときに運用上の役割を割り当てることで、すべてのチームがパイロットの運用タスクを開始し、展開の開始後にすべての準備ができていることを確認できます。
...
この役割に関連付けられたタスクとアクティビティは、運用ガイドで文書化されています。 この役割は[計画段階](https://aka.ms/SkypeToTeams-Plan)で割り当てる必要があります。 品質チャンピオンの役割を運用できる状態にする場合、その役割で必須となる知識を得ることと、前提条件がタスクで実現できる状態に確立されていることを確認することが主な手順になります。 この役割の主なタスクは、定期的な品質エクスペリエンスのレビューを実行することです。
...
[Teams または Skype for Business Online および Teams のプロキシサーバー](proxy-servers-for-skype-for-business-online.md)
