...
- **背景**。 Office 365 フローのスキャン、トラフィックの種類、顧客ネットワークから Office 365 サービスエンドポイントへの相互運用性、サードパーティ製コンポーネントとの相互運用性、およびチームがメディアフローを選択するために使用する原則などの背景情報を提供します。
...
ダイレクトルーティングでは、サポートされているオンプレミスのユーザー所有のセッションボーダーコントローラー (SBC) ハードウェアデバイスを Office 365 にペアリングし、そのデバイスにテレフォニートランクを接続して、サードパーティの公衆交換電話網 (PSTN) サービスプロバイダーを使用することができます。
