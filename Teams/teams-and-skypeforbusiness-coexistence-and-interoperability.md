...
チームは、共同作業機能だけでなく、チャット、通話、および会議機能を提供します。 チームを配置するを選択する方法によってこれらの機能は、ユーザーごとに、ビジネスの Skype によって提供される機能を備えた重なる場合があります。 業務用 Skype と共にチームを実行する既定のモードは、します。ただし、ユーザー指定できますこれらの機能は、そのユーザーの重なっていないことを確認するように設計されたいくつかの共存モードのいずれかです。
...
このモードを使用すると、ビジネスの Skype の既存の投資を活用しながらお客様の環境でチームを紹介します。 Skype チャット、通話、および会議の機能をそのままのビジネスのままにするこのモードで、チームの共同作業機能を追加して、Office 365、およびアプリケーション内のファイルへのアクセスをチームとのチャネル。

### <a name="skype-for-business-with-teams-collaboration-and-meetings-this-mode-is-upcoming"></a>チームの共同作業や会議が (このモードは、期限の迫っている) とビジネスの Skype

この共存モードを使用して、優れた品質、議事録などの新機能を活用する、チームの共同作業機能だけでなく、組織内の機能、ユーザーの有効性を促進し、翻訳、およびブラウザーで会議をサポートします。

チームと、このモードでの会話のチャネル ベースのチームを使用するとは、ユーザーは、チームを使用してスケジュールを設定し、会議の開催を開始します。 プライベート チャット、音声とビデオ通話は、Skype のビジネスのままになります。 この共存モードでは、Skype でのビジネス上の設置型展開を持つエンタープライズ ボイス、チームにアップグレードするいくつかの時間がかかる可能性のあるユーザーに特に便利です。
...
これらの相互運用機能のエクスペリエンスが割り当てられている共存モードは次のいずれかを持つユーザーが利用可能な:**チームの共同作業によるビジネスの Skype**、 **Skype**、**ビジネスの Skype のみ**、または**チームのみ**です。
