...
管理者は、検証と最小限の特権を *主張* し、妥協案を想定することで、ユーザーとデータの両方に対するリスクを最小限に抑えるアクションにつながる標準を使用して、安全な条件を作成できます。
...
1. オペレーティングシステムプラットフォームのセキュリティ機能を利用します。
    ...
    1. [**BitLocker**](/windows/security/information-protection/bitlocker/bitlocker-overview)などのデバイス暗号化機能が有効になっており、デバイスへのアクセスに使用されるキーがセキュリティで保護されていることを確認します。 最新の [**Windows 10 デバイスのほとんどは、BitLockerをサポートしています**](/windows/security/information-protection/bitlocker/bitlocker-device-encryption-overview-windows-10)。 
...
[Tems Web クライアント/ Teams Web アプリ](./get-clients.md#browser-client)
