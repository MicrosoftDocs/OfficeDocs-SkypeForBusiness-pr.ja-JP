...
> [!NOTE]
> Patient リソースは唯一の必須リソースです (アプリが読み込めなくても)。ただし、パートナーは、以下に示す仕様に従って上記のすべてのリソースのサポートを実装して、Microsoft Teams Patients アプリを使用した最適なエクスペリエンスを実現Microsoft Teams勧めします。

複数のリソースMicrosoft Teams Patients アプリからのクエリは、FHIR サーバーの URL に要求のバンドル (BATCH) を投稿する必要があります。 サーバーは各要求を処理し、各要求に一致するリソースのバンドルを返します。 詳細と例については、 を参照してください [https://www.hl7.org/fhir/STU3/http.html#transaction](https://www.hl7.org/fhir/STU3/http.html#transaction) 。
