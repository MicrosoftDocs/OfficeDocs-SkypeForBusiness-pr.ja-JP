...
- ![番号1のアイコン。前のスクリーンショット](../../media/sfbcallout1.png) **** の開封確認メッセージを参照すると、チャットメッセージの送信者は、受信者がメッセージを読んだことを確認できます。 この設定を使用して、すべてのユーザーに対して開封確認メッセージをユーザーが制御するか、すべてのユーザーに対して有効にするかを指定します。 メッセージの開封確認メッセージは、uncertainly を削除するために重要です。

  医療アプリケーションの場合は、[**ユーザーコントロール**] または [**有効**] を選択します。 [**すべてのユーザーに対して有効に**する] 設定を使用している場合は、テナント全体に対して受信確認を設定する唯一の方法は、テナント全体に対して1つのメッセージングポリシーのみを設定する方法 (既定のポリシーは "グローバル (組織全体の既定)" と呼ばれます)、またはすべてのメッセージングテナントのポリシーでは、受信確認に同じ設定が使用されます。

  > [!NOTE]
  > 大規模なグループチャットで開封済みメッセージを使用すると (100 ユーザーを超える場合など)、受信メッセージによって実際のメッセージがあふれ、チャットユーザーの不満を招く可能性があります。 これは、ユーザーに対して知っておく必要があるものです。 グループチャット (20 人以下のユーザなど) では、この機能をより効果的に使用できます。
