...
医療チームは、患者テンプレートを使用して開始できます。 リストを作成することで、チームは患者のニーズと状態を追跡できます。 Excel スプレッドシート上の既存の患者データを取り込んで、Teams でリストを作成することが可能です。 これらのリストは、回診や治療の連携を目的とした患者の監視などのシナリオで利用できます。
