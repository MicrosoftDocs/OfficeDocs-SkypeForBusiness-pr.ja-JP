...
現在、オーディオ/ビデオ (AV) を使用した VDI 上の Teams の最適化は、Windows Virtual Desktop、Citrix、VMware で認定されています。 適切な機能のすべての要件を満たしていることを確認するには、このセクションの情報を確認します。
...
チャットやコラボレーションに加えて、VDI 上の Teams と通話と会議を利用できます。サポートされている仮想化プロバイダー プラットフォームを使用できます。 サポートされる機能は、WebRTC メディア スタックと仮想化プロバイダーの実装に基づいて行います。 次の図では、このアーキテクチャの概要を説明します。
...
1. Microsoft Teams 管理センターの左側のナビゲーションで、**[ユーザー]** に移動し、ユーザーを検索するか、表示にフィルターを適用してユーザーを表示します。
2. [**&#x2713;** (チェックマーク)] の列からユーザーを選択します。 すべてのユーザーを選択するには、表 **&#x2713;** にあるチェック マーク (チェック マーク) をクリックします。
3. [**設定の編集**] をクリックし、必要な変更を行い、[**適用**] をクリックします。
...
- 現時点では、Windows Virtual Desktop および VLinux ベースのクライアントはサポートされていません。
...
- 新しい会議のエクスペリエンスや新しい会議エクスペリエンスに付属する機能など、複数ウィンドウの機能
- 拡張緊急サービス
...
- Skype for Business との相互運用性は、音声通話に制限されています。ビデオ モダリティはありません。
