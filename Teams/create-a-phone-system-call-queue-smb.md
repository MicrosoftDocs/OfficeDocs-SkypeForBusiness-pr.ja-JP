...
通話キューを作成するためのこの記事で説明されている手順では、Teams 管理センターを使用します。 PowerShell を使用して通話キューを作成する手順については、「PowerShell [コマンドレットを使用した通話キューの作成](create-a-phone-system-call-queue-via-cmdlets.md)」を参照してください。
...
1. [ **リソース アカウント**] で、[ **追加** ] ボタンを選択して、この呼び出しキューのリソース アカウントを追加します。
...
- [Microsoft Teams 電話 システム リソース アカウント ライセンスを](manage-resource-accounts.md#assign-a-license)割り当てます。
...
[Teams チャネルを使用してキューを管理](https://support.microsoft.com/office/9f07dabe-91c6-4a9b-a545-8ffdddd2504e)するには:

1. [チームの選択] ラジオ ボタン **を選択** し、[ **チャネルの追加**] を選択します。
1. 使用するチームを検索して選択し、[ **追加**] を選択します。
1. 使用するチャネル (標準チャネルまたはプライベート チャネルのみがサポートされている) を選択し、[適用] を選択 **します**。
...
> 
> プライベート チャネルにチーム メンバーのサブセットのみが含まれている場合でも、呼び出しはチームのすべてのメンバーに配布されます。
...
- Microsoft Teams 電話システム バージョン 1449/1.0.94.2020051601 以降。
  
エージェントの Teams アカウントをモードに設定する `TeamsOnly` 必要があります。 要件を満たしていないエージェントは、通話ルーティング リストに含まれません。 エージェントが互換性のあるクライアントを使用している場合は、通話キューの会議モードを有効にすることをお勧めします。
...
>
> 会議モードは、Skype for Business Serverからキューにルーティングされる呼び出しではサポートされていません。
> 
...
3. エージェントのルーティング方法で [最長アイドル時間] を選択すると、Presence-Basedルーティングが自動的に有効になります。
...
- Skype for Businessデスクトップ クライアント 2016 (32 ビットおよび 64 ビット バージョン)。
...
- Mac Skype for Business クライアント (バージョン 16.8.196 以降)。
- Android Skype for Business クライアント (バージョン 6.16.0.9 以降)。
- iPhone Skype for Business クライアント (バージョン 6.16.0 以降)。
- iPad Skype for Business クライアント (バージョン 6.16.0 以降)。
- Microsoft Teams Windows クライアント (32 ビットおよび 64 ビット バージョン)。
- Microsoft Teams Mac クライアント。
- [仮想化デスクトップ インフラストラクチャ](teams-for-vdi.md) (Windows Virtual Desktop、Citrix、VMware) 上の Microsoft Teams。
...
[Microsoft Teams 電話システムで得られる内容は次のとおりです](here-s-what-you-get-with-phone-system.md)
