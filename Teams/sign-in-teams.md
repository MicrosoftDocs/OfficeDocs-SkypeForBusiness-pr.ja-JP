...
## <a name="global-sign-in-and-sign-out"></a>グローバルなサインインとサインアウト

Teams Android アプリは、グローバルなサインインとサインアウトをサポートするようになり、第一線で活躍する人たちのための手間なしでサインインとサインアウトができるようになりました。 従業員は、共有デバイスプールからデバイスを選択し、1回サインインするだけで、シフト中は"自分のものにする" ということができます。 これらのユーザーは、シフトの最後に、サインアウトして、デバイスに対してグローバルにログアウトできます。 これにより、デバイスから個人情報と会社情報がすべて削除され、デバイスがデバイスプールに返されます。 この機能を利用するには、デバイスを shared モードにする必要があります。 共有デバイスをセットアップする方法の詳細については、「[Androidで共有デバイスモードを使用する方法](https://docs.microsoft.com/azure/active-directory/develop/tutorial-v2-shared-device-mode#set-up-an-android-device-in-shared-mode) 」を参照してください。

サインインエクスペリエンスは、Teamsの標準的なサインイン操作と似ていますが、サインアウトは次の2つの画像のようなイメージです。
...
新しい認証は、Teamsを使用するすべての組織で使用できます。 ユーザーがプロセスを完了できない場合は、ドメインまたは組織の Microsoft の職場または学校のアカウントに問題がある可能性があります。
