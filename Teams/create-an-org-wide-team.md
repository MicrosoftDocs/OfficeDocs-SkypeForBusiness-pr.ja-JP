...
組織全体にわたるチームでは、グローバル管理者はパブリックのチームでは、組織内のすべてのユーザーと最新のメンバーシップ ユーザーの結合と Active Directory の組織のままにするを簡単に作成できます。 グローバル管理者のみが組織全体にわたるチームを作成できる、現在、組織全体にわたるチームは 5,000 人のユーザーを持つ組織に限定します。 これらの要件を満たしている場合のオプションとして選択したとき**最初からチームを構築する**チームを作成するときにグローバル管理者に**組織全体**が表示されます。 
...
> [!NOTE]
> - チームを作成するときに、[**組織全体**] オプションが表示されないし、あなたは、グローバル管理者、機能のロール アウトするも可能性がありますか、組織がメンバー数が 5,000 の現在のサイズ制限を超える必要があります。 将来的にこの制限を大きくには求めています。
...
### <a name="is-there-a-way-to-convert-an-existing-team-to-an-org-wide-team"></a>既存のチームを組織全体にわたるチームに変換する方法はありますか。

グローバル管理者は、既存のチームをチームのクライアントで編集して、組織全体にわたるチームに変換できます。
チーム名に移動し、[詳細オプション] をクリックしてください._gt 編集チームは (省略記号)。
