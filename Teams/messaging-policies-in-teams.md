...
- **送信されたメッセージを削除する** ユーザーがチャットで送信したメッセージを削除するには、この設定を使用します。
