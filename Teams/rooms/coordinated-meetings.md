...
2. [ **Users**  >  **アクティブな**ユーザーのユーザーに移動する。
3. [ **表示名** ] 列で、チームルームデバイスまたは Surface Hub の名前を見つけます (多数のユーザーがいる場合は、 **検索** ボックスを使用できます)。
4. [ **Username** ] 列で UPN を検索します (alias@contoso.com や alias@contoso.onmicrosoft.com などのようになります)。
...
3. コーディネートした**会議**を選ぶ
4. [ **オプション**] で、[ **調和のとれた会議** ] を [オン] に設定します
5. ワークシートに **オーディオの既定値** が設定されている場合は `true` 、[ **このデバイスのマイク** をオンにする] をオンにします。それ以外の場合はオフにします。
...
7. ワークシートの **既定のビデオ** の場合は `true` 、[ **このデバイスのカメラ** をオンにする] をオンにし、それ以外の場合はオフにします。
8. ワークシートで**ビデオが有効になって**いる場合は `true` 、[**このデバイスのカメラをオン**にしている**ときに会議に参加するときにユーザーに**許可する] を選択します。 **このデバイスの [カメラ**] がオンに設定されている場合は、このオプションを無効にできません。
9. **ホワイトボードの既定値**がワークシートにある場合は `true` 、[**このデバイスの whiteboarding**をオンにする] をオンにし、それ以外の場合はオフにします。
...
3. Upn のリストに変更してください。 `TrustedAccounts`
4. ファイルに名前を付けて保存する `SkypeSettings.xml`
