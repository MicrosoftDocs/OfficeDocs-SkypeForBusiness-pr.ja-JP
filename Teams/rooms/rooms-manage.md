...
組織内に Microsoft Teams Rooms 認定デバイスがある場合は、Microsoft Teams 管理センターを使用して中央の場所からデバイスを管理できます。 次の操作を実行できます。

- デバイスの再起動やブロック、デバイスログのダウンロードなどのデバイス管理を実行する
- Teams 固有の設定を適用する
- Microsoft Teams ミーティング デバイスとその周辺機器（カメラ、ディスプレイ、マイクなど）の正常性の状態を確認する
- 現在と過去のミーティングアクティビティ（通話品質、ネットワークの状態と接続、参加者数など）を確認する
- Microsoft Teams ミーティング デバイスに接続されている周辺機器（カメラやプロジェクターなど）を表示する
...
> [!IMPORTANT]
> Teams 管理センターを使用してデバイスを管理するには、Teams サービス管理者または全体管理者の役割が割り当てられている必要があります。
