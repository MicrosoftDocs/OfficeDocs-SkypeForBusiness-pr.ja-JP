...
> [!NOTE]
> TEAMS アプリなしでエージェントをインストールして、TEAMSにログインできる場合は、オプションのプロセスとして登録キーを使用できます。 [?] に移動します。 (ヘルプ) ポータルの右上隅にある [ダウンロード キー (省略可能)] を選択します。 エージェントをインストールするときは、デバイスの  **C:\Rigeldirectory**  に "セルフ登録キー" (以前にポータルからダウンロードしたキー) を配置します。
...
1. 管理者としてデバイスにログインします。 デバイスの手順 *の管理者ユーザーとして実行操作が* 実行されていることを確認します。
