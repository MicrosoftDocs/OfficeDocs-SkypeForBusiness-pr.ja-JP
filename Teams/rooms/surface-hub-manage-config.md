...
> [!IMPORTANT]
> Surface Hubs をデバイス グループに含め、構成プロファイルを適用するMicrosoft Intuneデバイスを特定できる必要があります。 デバイス グループを作成する方法については、「グループを追加してユーザーとデバイスを整理 [する」を参照してください](/mem/intune/fundamentals/groups-add)。

Surface Hubs に設定を適用する構成プロファイルをTeamsするには、次の操作を行います。
...
7. [行 **の追加] ウィンドウ** で、次の操作を行います。
    1. わかりやすい名前を付け、必要に応じて、追加するTeamsの説明を入力します。
