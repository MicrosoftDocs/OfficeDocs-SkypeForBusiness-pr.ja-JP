...
ms.locfileid: "67270702"
---
# <a name="use-power-bi-to-analyze-cqd-data-for-microsoft-teams"></a>Power BI を使用して Microsoft Teams の CQD データを分析する

2022 年 7 月の新機能: 新しい Quality of Experience (QER) テンプレートが [、CQD ダウンロード用の Power BI クエリ テンプレート](https://www.microsoft.com/download/details.aspx?id=102291)に追加されました。 QER は、2020 年にリリースされた元の CQD Power BI クエリ テンプレートを置き換える強力なレポート テンプレートです。 元のテンプレートはデモンストレーション目的で引き続き使用できますが、サポートされなくなり、引き続き更新プログラムを受け取る QER テンプレートに切り替えてお勧めします。 これは、CQD Teams 自動応答&通話キュー履歴レポートには適用されないことに注意してください。
...
|CQD PSTN ダイレクト ルーティング レポート.pbit <br/> *(非推奨)*    | ダイレクト ルーティングを経由する PSTN 通話に固有の分析情報を提供するテンプレートの例。 詳細については、「 [CQD PSTN ダイレクト ルーティング レポートの使用」](CQD-PSTN-report.md)を参照してください。 |
|CQD Helpdesk Report.pbit <br/> *(非推奨)*     |建物と EUII のデータを統合して、このテンプレートは、1 人のユーザーからのドリルスルーを有効にして、そのユーザーの呼び出し品質が低いアップストリームの根本原因を見つける方法を示します (たとえば、ユーザーがネットワークの問題が発生している建物にいる場合など)。 |
|CQD Location Enhanced Report.pbit <br/> *(非推奨)*     | このテンプレートの例では、CQD SPD の場所レポートを再想像します。 9 つのレポートが含まれており、通話品質、建物の WiFi、信頼性、および通話のレート (RMC) の情報を、ビルドまたはユーザー別に追加のドリルスルーで提供します。 レポート エクスペリエンスを最大限に高めるために、必ず建物データをアップロードしてください。 |
|CQD モバイル デバイス Report.pbit <br/> *(非推奨)*     | このテンプレートの例では、通話品質、信頼性、通話の評価など、モバイル デバイス ユーザー向けに特別に調整された分析情報を提供します。 モバイル ネットワーク、WiFi ネットワーク、モバイル オペレーティング システムのレポート (Android、iOS) を表示します。 |
|CQD の概要レポート.pbit <br/> *(非推奨)*    | CQD と Power BI を組み合わせることで、視覚化の向上、プレゼンテーションの改善、情報密度の向上、およびローリング日付を提供する方法のデモンストレーション。 これらのレポートを使用すると、外れ値を識別しやすくなります。 使いやすい対話型マップを使用して、場所ごとの通話品質を掘り下がります。 9 つの新しいレポート:</p>- 全体的な品質<br>- 信頼性全体<br>- RMC (通話のレート) 全体<br>- 会議の品質<br>- P2P 品質<br>- 会議の信頼性<br>- P2P の信頼性<br>- 電話会議 RMC<br>- P2P RMC         |
|CQD ユーザー フィードバック (通話のレート) Report.pbit <br/> *(非推奨)*    | 組織の呼び出しをサポートするために簡単に使用できる方法で、Rate My Call データを使用した Power BI レポートのデモンストレーション。 エンド ユーザー教育の機会を特定するための詳細を含む相互参照。 |
