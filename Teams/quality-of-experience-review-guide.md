...
"パフォーマンスが低い既知のクライアントバージョンが使用されています" という診断アラートが表示された場合は、最初にそれらのクライアントの更新に注目します。 このアラートは、これらのクライアントが通話品質に悪影響を与えていることを確認しました。 既知の問題のあるクライアントが更新されなくなったことを確認するために、クライアント & デバイスレポート (上記) を使用できます。
...
"オーディオドライバーが原因で通話が切断されています" という診断アラートが表示された場合は、最初にを修復するにフォーカスします。 このアラートによって、既知の不正なドライバーが原因で通話が切断され、通話の信頼性に悪影響が生じていることが判明しました。 このプロセスについては、「クライアント & デバイス」セクションで説明されているマイクドライバーレポート (上記を参照) を使用できます。
