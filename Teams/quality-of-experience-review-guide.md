...
-   **Skype for Business の管理者の役割** 

-   **Teams サービス管理者の役割** 

-   **Teams 通信サポートエンジニアロール** 

-   **Teams コミュニケーションサポートスペシャリストロール** 

または、次の役割を Office 365 ユーザーアカウントに割り当てて、レポート機能のみにアクセスできるようにすることもできます。

-   **レポートリーダー:** Office 365 管理センターですべての[アクティビティレポート](https://support.office.com/article/activity-reports-0d6dfb17-8582-4172-a9a9-aed798150263)を表示できます。また、 [Microsoft 365 の導入コンテンツパック](https://support.office.com/article/Office-365-Adoption-content-pack-77ff780d-ab19-4553-adea-09cb65ad0f1f)と CQD レポートからすべてのレポートを表示できます。
...
| 非標準的なサンプル\>の平均比 7% または0.07 | パケット損失の回復によって生成された、オーディオフレームの合計数に対する、非表示のサンプルを含むオーディオフレームの平均比率。 隠しオーディオのサンプルは、ネットワークパケットのドロップによって通常発生する突然の切り替えをスムーズにするために使用される手法です。      | 値が大きい場合は、大量の損失 concealment が適用され、音声がひずむか失われることを示します。     |
...
通話料金 (RMC) は Teams と Skype for Business に組み込まれており、参加者が10回、または 10% の間隔で参加者に表示されるように自動的に構成されます。 この簡単なアンケートを行うと、ユーザーは通話の評価を行い、通話品質が悪い可能性がある理由について若干のコンテキストを提供するように求められます。 1つまたは2つの評価は低品質で、3 ~ 4 は良好であり、5は素晴らしいと考えられます。 これは、鈍くなるインジケーターの一部ですが、サービスの測度が見逃される問題を明らかにするための便利な指標となります。
...
![1 つの品質カテゴリあたりの週間タスクの一覧](media/qerguide-image-tasks.png "1 つの品質カテゴリあたりの週間タスクの一覧")

#### <a name="service-management-tasks"></a>サービス管理タスク

クラウドの最初の世界では、高品質のユーザーエクスペリエンスを維持するために特定のサービス管理タスクを実行する必要があります。 これらのタスクには、インターネットリンクを飽和させずにサービスに接続するための十分な帯域幅があることを保証し、すべての管理されたネットワーク領域にサービスの品質 (QoS) が設定されていることを確認することや、最後に[Office 365 IP 範囲を常に把握しておくことなどがあります。ファイアウォール](https://aka.ms/o365ips)。
...
> [!Note]
> CQD には、個人を特定できる情報 (PII) が含まれていません。 PII は、単独で使用したり、1人のユーザーを特定、連絡、検索したり、コンテキストで個人を特定するために他の情報と共に使用したりすることができる情報です。
...
-   CQD には PII が含まれていません。

### <a name="report-editions"></a>レポートエディション

CQD Online には、"概要" と "詳細" という2つのレポートエディションがあります。 画面上部の青いバーにあるドロップダウンメニューを使用して、レポートエディションを開きます。 選択したレポートエディションの名前が画面の上部に表示されます。
...
|             |           |
| ------------|-----------|
| ![前のスクリーンショットで吹き出しを参照する数値1のアイコン](media/qerguide-image-callout1.png "いずれか") | [概要] ウィンドウには、右側に表示されるレポートセットのコンテキストが表示されます。 |
| ![前のスクリーンショットで吹き出しを参照している数値2のアイコン](media/qerguide-image-callout2.png "2") | [概要] ウィンドウで [**編集**] を選ぶと、レポートレベルのプロパティ (y 軸の高さを含む) を設定したり、新しいテンプレートをインポートしたりできます。 |
| ![前のスクリーンショットで吹き出しを参照している数値3のアイコン](media/qerguide-image-callout3.png "3 つ") | 階層リンクを使用すると、ユーザーはレポートセット階層内の現在の位置情報を特定するのに役立ちます。 |
| ![前のスクリーンショットで吹き出しを参照している数値4のアイコン](media/qerguide-image-callout4.png "4") | 子レポートを含むレポートは、青色のリンクで示されます。 リンクを選択すると、子レポートにドリルダウンすることができます。 |
...
|             |           |
| ------------|-----------|
| ![前のスクリーンショットで吹き出しを参照する数値1のアイコン](media/qerguide-image-callout1.png "いずれか") | 左側のウィンドウからディメンション、メジャー、フィルターを選択します。 既存の値をポイントすると、[閉じる] ボタン (**X**) が表示され、この値を削除することができます。<ul><li>寸法またはメジャーを選択することにより、タイトルフィールドを編集**** してタイトルを変更することができます。 上部のウィンドウで青色の上向き矢印または下向き矢印を選択して順序を変更することもできます。</li><li>見出しの**+** 横にある () を選択すると、新しい寸法、メジャー、またはフィルターを追加するためのダイアログボックスが開きます。</li><li>検索を簡単にするには、[フィールドの**検索**] フィールドに、ディメンション、メジャー、またはフィルターの最初の数文字を入力します。</li></ul> |
| ![前のスクリーンショットで吹き出しを参照している数値2のアイコン](media/qerguide-image-callout2.png "2") | 上のウィンドウには、グラフをカスタマイズするためのオプションが表示されます。 |
| ![前のスクリーンショットで吹き出しを参照している数値3のアイコン](media/qerguide-image-callout3.png "3 つ") | クエリエディターにレポートのプレビューが表示されます。 |
| ![前のスクリーンショットで吹き出しを参照している数値4のアイコン](media/qerguide-image-callout4.png "4") | レポートの詳細な説明を作成または編集するには、画面の下部にある [**編集**] ボックスを使用します。 |
...
-   各行には14個の列がある必要があります。 各列には、次の表で説明されているデータ型が必要です。また、列はテーブルの順序で指定する必要があります。
...
> [!IMPORTANT]
> 特定の VPN 実装では、サブネット情報を正確に報告しません。 この問題が報告で発生した場合は、サブネットのエントリの1つではなく、建物のファイルに VPN サブネットを追加するときに、VPN サブネットの各アドレスに個別のエントリを個別の32ビットネットワークとして追加することをお勧めします。 各行には、同じビルドメタデータを含めることができます。 たとえば、172.16.18.0/24 の1行ではなく、253行には、172.16.18.1/32 ~ 172.16.18.254/32 のアドレスごとに1行の行があります。
...
ネットワーク上にメディアのワークロードを展開して全体的な影響を評価する場合は、ネットワークプランナーを活用することを強くお勧めします。 ネットワークプランナーの詳細については、 [Myadvisor](https://myadvisor.fasttrack.microsoft.com/CloudVoice/NetworkPlanner)にアクセスしてください。


## <a name="diagnostic-alerts"></a>診断アラート

Microsoft は、CQD テレメトリを事前に監視して、ユーザーエクスペリエンスに悪影響を及ぼす既知の問題に関する診断アラートを作成します。 これらの通知は、メッセージセンターを介して自動的にサービス管理者に配信されます。 次の表に、メッセージセンターに表示される診断アラートと、詳細情報へのリンクを示します。

_表 6-診断アラート_

| 通知                                                                | 詳細情報             |
|----------------------------------------------------------------------|------------------------------|
| 既知のパフォーマンスの低いクライアントバージョンが使用されている                   | [クライアントのバージョン](#client-versions)              |
| オーディオドライバーが原因で通話が切断される                                 | [Devices](#devices)                      |
| ファイアウォールの制限により、通話のセットアップエラーが発生する         | [セットアップ失敗の調査](#setup-failure-investigations) |
| ディープパケット検査のため、通話のセットアップエラーが発生しています                | [セットアップ失敗の調査](#setup-failure-investigations) |
| Wi-fi ネットワーク上の会議室のデバイスが、通話品質の低下を引き起こしています | [品質調査](#quality-investigations)       |
| UDP トラフィックが制限されているため、通話品質の低下が発生する         | [TCP](#tcp)                          |
| VPN の利用状況が通話品質に影響しています                                  | [品質調査](#quality-investigations)       |


### <a name="message-center"></a>メッセージセンター

メッセージセンターでは、新しい更新、機能、問題についての警告が表示されます。 メッセージセンターは、サービス管理者が Microsoft 365 管理センター内で利用できます。 各投稿には、更新プログラム、機能、問題がユーザーにどのように影響するか、詳細情報へのリンクが記載されています。

メッセージセンターを開くには、Microsoft 365 管理センターで、[**正常性** > **メッセージセンター**] に移動するか、[**ホーム**] ダッシュボードのメッセージセンターカードを選択します。 ダッシュボードには、投稿された最後の3つのメッセージと、[フルメッセージセンター] ページへのリンクが表示されます。
...
モバイルデバイスで[Office 365 Admin アプリ](https://go.microsoft.com/fwlink/p/?linkid=627216)を使用してメッセージセンターを表示することもできます。これは、プッシュ通知を使用して最新の状態を維持する優れた方法です。 詳細については、[この記事](https://support.office.com/article/Message-center-in-Office-365-38FB3333-BFCC-4340-A37B-DEDA509C2093)を参照してください。
...
##### <a name="diagnostic-alert"></a>診断アラート

"ファイアウォールによって通話のセットアップエラーが発生しています" という診断アラートが表示された場合、または "詳細なパケット検査によって通話のセットアップエラーが発生しました" というメッセージが表示された場合は、最初にこれらの識別されたサブネットの このアラートは、通話の信頼性に悪影響を及ぼすサブネットを特定しています。 [信頼性] セクションにある [セットアップエラー] レポートを使用して、修復に役立てることができます。
...
問題の領域が見つかったら、[通話分析](https://techcommunity.microsoft.com/t5/Skype-for-Business-Blog/Introducing-Call-Analytics/ba-p/57309)を使用して、その建物のユーザーに対して、特定の問題をさらに詳しく確認できます。 通話分析には PII データが含まれており、これを使用すると、ドロップが失敗する可能性がある理由をさらに切り分けることができます。
...
| **ユーザーの動作**                            | ネットワーク、デバイス、またはクライアントのどちらも問題ではないと判断した場合は、ユーザーを教育して、会議に参加して終了する方法をユーザーに教えるためのガイダンスについては、[アドバイザー](https://aka.ms/myadvisor)を利用することを検討してください。 スマートなチームと Skype for Business のユーザーは、会議のすべての参加者に対してユーザーエクスペリエンスを向上させることができます。 会議を終了せずにノート pc をスリープ状態にする (ふたを閉じることで) ユーザーは、予期しない通話のドロップとして分類されます。   |
...
PSR パーセンテージは、組織が特定のフォーカス領域に対して定義されたメトリック目標を満たしているかどうかを示すために使用されます。 上位レベルのパーセンテージが定義されたターゲット内にある場合でも、個々のサブネットまたは建物が定義されたターゲットを満たしていない可能性があるため、さらに調査する必要があることに注意してください。 たとえば、全体的なオーディオ PSR パーセンテージが4月の4パーセントになっている場合、その目標を満たしている場合、個々の建物およびサブネットの品質が、その 2% の全体的な分布によっては依然として不十分な場合があります。 
...
Wi-fi ネットワーク上での会議室デバイスが、通話品質の低下の原因となっている場合は、次のようにして、修復作業でそれらのデバイスを含めて優先順位を付けます。 このアラートは、通話品質の低下に積極的に貢献している Wi-fi 上の会議室を特定しました。

"VPN の使用状況が通話品質に影響しています" という診断アラートが表示された場合は、分離トンネリングソリューションを調査して VPN アプライアンスをバイパスし、メディアがサービスに直接接続することを許可します。 このアラートにより、VPN が通話品質に悪影響を与えることが判明しました。
...
| **VPN**  | VPN アプライアンスは、従来はリアルタイムのメディアワークロードを処理するように設計されていません。 一部の VPN 構成では、UDP (メディアの優先プロトコル) を使用できないため、TCP のみを利用することは禁じられています。 Vpn[分割トンネルソリューション](https://myadvisor.fasttrack.microsoft.com/CloudVoice/Downloads?SelectedIDs=5_1_0_9)を実装して、vpn を低品質のソースとして減らすことができます。 |
| **クライアント** <br>(Skype for Business Online のみ) | 以前のクライアントでは、メディアの問題の原因となることがわかっています。 6ヶ月のリリースからクライアントへの修正プログラムが適用されていることを確認します。 クライアントの準備戦略の開発および[クイック実行](https://docs.microsoft.com/DeployOffice/deployment-guide-for-office-365-proplus)の展開に関するガイダンスについては、 [myadvisor](https://aka.ms/myadvisor)を使用します。 |
| **Devices** | 最適化された[デバイス](https://partnersolutions.skypeforbusiness.com/solutionscatalog)を使用すると、ユーザーエクスペリエンスを大幅に向上させることができます。 すべてが同等で、最適化されたデバイスは、チームと Skype for Business のユーザーエクスペリエンスを最大限に高め、優れた品質を生み出すように設計されています。 [Myadvisor](https://aka.ms/myadvisor)を使って、デバイスの準備戦略の開発に関するガイダンスを行います。 |
...
"UDP トラフィックは制限されていますが、通話品質が悪くなります" という診断アラートを受け取った場合は、まず、それらのサブネットで TCP の修復作業に重点を置いてください。 アラートは、TCP の使用が通話品質に悪影響を及ぼすサブネットを特定しています。
...
プロキシをバイパスできない組織の場合、skype for business クライアントがプロキシの背後にあるときに適切にサインインするように構成されていることを確認します。「Skype for Business では、[直接ログインする代わりにプロキシサーバーを使用する」をお勧めします。接続](https://support.microsoft.com/help/3207112/skype-for-business-should-use-proxy-server-to-sign-in-instead-of-tryin)。 
...
Skype for Business Online および Teams でのプロキシの使用の詳細については、こちらの[記事](proxy-servers-for-skype-for-business-online.md)を参照してください。
...
"パフォーマンスが低い既知のクライアントバージョンが使用されています" という診断アラートが表示された場合は、最初にそれらのクライアントの更新に注目します。 このアラートは、これらのクライアントが通話品質に悪影響を与えていることを確認しました。 既知の問題のあるクライアントが更新されていないことを確認するには、Client & Devices レポート (上記に示す) を使用できます。
...
デバイスレポートは、ボリュームと MOS score (オーディオのみ) によるデバイスの使用状況を評価するために使用され、[クライアント & デバイス] の下にある付随するテンプレートで確認できます。 
...
"オーディオドライバーが原因で通話が切断されています" という診断アラートが表示された場合は、最初にを修復するにフォーカスします。 このアラートによって、既知の不正なドライバーが原因で通話が切断され、通話の信頼性に悪影響が生じていることが判明しました。 このプロセスについては、「クライアント & Devices」セクションで説明されているマイクドライバーレポート (上記を参照) を使用できます。
...
次のタスクは、認定された[オーディオデバイス](https://partnersolutions.skypeforbusiness.com/solutionscatalog/personal-peripherals-pcs)の全体的な使用状況を特定することです。 すべてのオーディオストリームで少なくとも 80% が認定されたオーディオデバイスを使用することをお勧めします。 これを実現するには、マイクデバイスレポートを Excel にエクスポートして、認定されたデバイスまたは承認済みデバイスの使用量を計算することをお勧めします。 通常、組織はすべての承認済みデバイスの一覧を保持するため、データのフィルター処理と並べ替えは簡単に行う必要があります。
...
| パケット損失                       | \<15秒間隔中は 1%    | \<15秒間隔で 0.1%  |
| パケット到着間ジッター       | \<15秒の間隔で30ミリ秒 | \<15秒間の間隔で15ミリ秒 |
...
-   [Skype for Business ソリューションのカタログパーソナルペリフェラル & Pc](http://partnersolutions.skypeforbusiness.com/solutionscatalog/personal-peripherals-pcs)
