...
-   **サービス管理:** このカテゴリは、次の2つのセクションで構成します。
    ...
    -   次に、インフラストラクチャがサービスに追加されたときに、組織が管理し、新しい Microsoft 365 または Office 365 IP アドレス用のファイアウォールの維持など、サービスへの確実なアクセスを確実に行う必要のあるタスクです。
...
クラウドの最初の世界では、高品質のユーザーエクスペリエンスを維持するために特定のサービス管理タスクを実行する必要があります。 これらのタスクには、インターネットリンクを飽和させずにサービスに接続するための十分な帯域幅があることを保証し、すべての管理されたネットワーク領域にサービスの品質 (QoS) が設定されていることを確認すること、そして最後に、[ファイアウォールで Microsoft 365 および Office 365 IP 範囲](https://aka.ms/o365ips)を常に把握することです
...
2. Azure PowerShell コマンドウィンドウを開き、次のスクリプトを実行します。メッセージが表示されたら、Microsoft 365 または Office 365 の資格情報を入力します。
...
個々の詳細レポートにフィルターを適用するには、 ``Is Teams`` レポートにフィルターを追加して、True または False に設定します。 詳細については、このセクションで後述する「[レポートの編集](#editing-reports)」を参照してください。
...
-   次の図に示すように、従来の**Skype for business 管理センター**ツールに移動して、  >  **tools**CQD へのリンクを選択します。
...
|             |           |
| ------------|-----------|
| ![前のスクリーンショットで吹き出しを参照する数値1のアイコン](media/qerguide-image-callout1.png "間") | 左側のウィンドウからディメンション、メジャー、フィルターを選択します。 既存の値をポイントすると、[閉じる] ボタン (**X**) が表示され、この値を削除することができます。<ul><li>寸法またはメジャーを選択することにより、タイトルフィールドを編集し**てタイトルを**変更することができます。 上部のウィンドウで青色の上向き矢印または下向き矢印を選択して順序を変更することもできます。</li><li>**+** 見出しの横にある () を選択すると、新しい寸法、メジャー、またはフィルターを追加するためのダイアログボックスが開きます。</li><li>検索を簡単にするには、[フィールドの**検索**] フィールドに、ディメンション、メジャー、またはフィルターの最初の数文字を入力します。</li></ul> |
...
> [!NOTE]
> 上の URL 例は、視覚的な表現のみを対象としています。 のデフォルトの CQD リンクを使用してください <https://cqd.teams.microsoft.com> 。
...
2. Microsoft 365 または Office 365 管理者の資格情報を使用して認証します。
...
1. を参照して、CQD Online に移動 <https://cqd.teams.microsoft.com> します。
...
![TCP 図25を使ったオーディオストリームの割合を示すグラフ、 ](media/qerguide-image-audiostreamswithtcp.png)
 _tcp 使用のオーディオストリーム_
...
### <a name="network-connectivity-principles"></a>ネットワーク接続の原則

Microsoft 365 または Office 365 ネットワーク接続のためにネットワークの計画を開始する前に、接続の原則を理解して、Microsoft 365 または Office 365 トラフィックを安全に管理し、最高のパフォーマンスを実現することが重要です。 次の記事は、ネットワーク接続を安全に最適化するための最新のガイダンスを理解するのに役立ちます。
