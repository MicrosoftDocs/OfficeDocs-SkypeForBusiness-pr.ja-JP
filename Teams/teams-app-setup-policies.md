...
> [!NOTE]
> 割り当てMicrosoft Teams for Educationが既定でグローバル ポリシーに固定されている場合でも、現時点ではグローバル ポリシーに表示されません。 これは、クライアント上のピン留めされたアプリの一覧で 4 Teamsされます。
...
4. アプリをピン留めしてユーザーがアプリ バーをカスタマイズできるかどうかに応じて、[ユーザーの固定を許可する] をオンまたはオフにします。
   ...
   > [!NOTE]
   > [ユーザーのピン留めを許可する] 設定は、Microsoft 365 Government Community Cloud (GCC) 環境 (GCC、GCC High、DoD) の Teams 管理センターで使用できますが、現在は無効です。
...
|![デスクトップ クライアントのTeamsアプリ](media/app-setup-policies-desktop-more-apps.png)<br>   |![モバイル クライアントのアプリTeamsする](media/app-setup-policies-mobile-more-apps.png)  
...
[ユーザーのピン留めを許可する] オプションがオンになっているTeams、デスクトップクライアントとモバイル クライアント上の固定アプリの順序を変更できます。 ユーザーは、Web クライアントでピン留めされたアプリのTeams変更することはできません。
