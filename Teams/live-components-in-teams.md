...
ms.locfileid: "60911861"
---
# <a name="manage-live-components-in-teams"></a>アプリケーションでライブ コンポーネントを管理Teams

チャット内のライブ Teamsは、一緒にアイデアを作成し、決定を下す新しい方法を提供します。 表、タスク リスト、段落などのコンポーネントを送信します。このコンポーネントでは、チャット内のすべてのユーザーがインラインで編集を行い、変更内容を確認できます。 つまり、会議を開催し、長いチャット スレッドの必要性を最小限に抑えながら、チームからアイデアやフィードバックを収集できます。
> [!Note]
...
## <a name="checking-if-the-fluid-framework-is-enabled-through-the-sharepoint-online-powershell-cmdlet"></a>流動フレームワーク Online PowerShell コマンドレットを使用してSharePointが有効になっているか確認する
...
## <a name="enabling-the-fluid-framework-through-the-sharepoint-online-powershell-cmdlet"></a>SharePoint 流動フレームワーク Online PowerShell コマンドレットを使用してSharePointを有効にする
...
- チャット内のライブ コンポーネントは、Android で Office アプリを使用Teamsを使用して編集することはできません。
...
- テナントの条件付きアクセス ポリシー設定でクライアント ネットワークが に接続できない場合、ライブ コンポーネントは、サービスに対するリアルタイムの変更を節約するために期待したほど `https://pushchannel.1drv.ms` 動作しません。
...
- B2B ゲストは、B2B ゲストがテナント メンバーと同じアクセス レベルを持つ外部アクセス オプションをテナントに設定しない限り、会社の共有リンクを介して共有されるライブ コンポーネントで共同作業を行う必要があります。

- TeamsWeb クライアントの Live コンポーネントの完全なサポートは間もなく提供される予定です。
...
1. [] に移動[Microsoft 365 管理センター。](https://admin.microsoft.com/)

2. 左側の [管理センター]**で、[管理****センター]** を選択SharePoint。
...
`.fluid`ファイルを開くことができるのは、Office.com などのブラウザーのリンクと、チャット内のライブ コンポーネントTeamsです。 ダウンロードした場合、最初にオンラインまたはオンラインにアップロードし直OneDrive for Business開SharePointできません。
