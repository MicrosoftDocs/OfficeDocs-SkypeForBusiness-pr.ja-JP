...
> [!NOTE]
> SIP ヘッダーには、使用されている SIP URI に userinfo は含されません。 RFC [3261 のセクション 19.1.1](https://tools.ietf.org/html/rfc3261#section-19.1.1)に基いて、URI の userinfo 部分は省略可能であり、宛先ホストにユーザーの考え方が存在しない場合や、アドレス自体が識別されるリソースである場合には存在しない可能性があります。 SIP URI に @ 記号が存在する場合、ユーザー フィールドを空にすることはできません。
...
*"名前には、任意の 1 つのドメイン名コンポーネントまたはコンポーネントフラグメントと一致すると見なされるワイルドカード \* 文字が含まれている可能性があります。例: .a.com は foo.a.com 一致 bar.foo.a.com。f .com は一致 foo.com が、一 \* \* 致 bar.com。*
...
- Microsoft SIP プロキシは、送信 ping オプションを送信するときに、(レコード ルートではなく) 連絡先ヘッダーのみを使用して、次のホップを決定します。 2 つのパラメーター (連絡先とレコード ルート) の代わりに 1 つのパラメーター (連絡先) のみを構成すると、プロキシ SBC が使用されていない場合の管理が簡素化されます。 
...
- [セッション開始プロトコル (SIP) の "参照者" メカニズム](https://tools.ietf.org/html/rfc3892)
...
- History-Info呼び出しと呼び出しの転送ケースのヘッダーが追加されます。
