...
- **ファイアウォールを開き、Microsoft 365をTeams。** ネットワークのファイアウォールを開き、Microsoft 365 URL Teams IP アドレス範囲に関するページで説明Office 365[トラフィックを送信します](/microsoft-365/enterprise/urls-and-ip-address-ranges)。

- **SIP デバイスがプロキシの背後に接続されていないか確認します。** http/s トラフィックが企業の http/s プロキシをバイパスします。

- **UDP ポートを開きます。** 49152 ~ 53247 の範囲で UDP ポートを開きます。
...
管理センターで SIP ゲートウェイを有効Teams、次の手順に従います。
...
3. [設定 **Web** ユーティリティの [詳細設定] で、上記のプロビジョニング サーバーの URL を入力します。 
...
3. 右上にある [アクション] [デバイスの **プロビジョニング**]  >  **を選択し**、次のいずれかの手順に従います。
  ...
  - **1 つのデバイスをプロビジョニングするには:**
   ...
   - **多くのデバイスをプロビジョニングするには:**
     ...
     b. [デバイス **のプロビジョニング] ウィンドウ** の [複数の **MAC アドレスアップロード] で**、[テンプレートのダウンロード]**を選択します**。
...
デバイスのユーザーが少なくとも 1 回サインインした後、Teamsセンターで SIP デバイスのインベントリを表示および監視できます。 次の操作を実行してください。
...
SIP ゲートウェイは IPv4 のみをサポートします。 Microsoft Teamsとクライアントは、IPv4 と IPv6 の両方をサポートします。 ネットワークへの通信を制御する場合Microsoft Teams URL と IP アドレス範囲で IP [Microsoft 365を使用します](/microsoft-365/enterprise/urls-and-ip-address-ranges)。
