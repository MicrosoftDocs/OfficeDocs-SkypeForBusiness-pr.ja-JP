...
7. [発行 **] を** 選び、カスタマイズしたアプリを公開します。
   ...
   カスタマイズしたアプリが [アプリの管理] ページ **に表示** されます。 アプリ機能をカスタマイズしてもアプリのコピーは作成できないので、アプリのバージョンは 1 つのみです。
...
> [!Note]
> - アプリとアプリに関連する説明をカスタマイズする場合は、アプリ発行元が提供するカスタマイズ ガイドラインに従う必要があります。 お客様は、お客様が使用する第三者の画像に関して、他のユーザーの権利を尊重する責任があります。
> - 管理者が提供するカスタマイズ データは、最も近い地域の [設定] ストアに保存されます。 GoLocal Cloud Teams の展開に必ずしも含めりません。
> - お客様は、使用条件またはプライバシー ポリシーへのリンクが有効か確認する責任があります。 アプリのメタデータに対して行った変更 (または許可) を適切に管理する必要があります。 現在の実装では、開発者が提供する URL に戻すサポートが提供されます。 URL なしでアプリを構成できない (アプリで URL のカスタマイズが許可されている場合)。
...
4. Publisher の元のアプリ名の短い名前など、 **アプリの詳細を表示します**。
...
カスタマイズできるのは、アプリの発行元によってカスタマイズが許可されているアプリのみです。 アプリの発行元は、お客様がアプリをカスタマイズすることを許可するためにオプトインする必要があります。
