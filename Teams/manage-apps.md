...
ms.locfileid: "66958062"
---
# <a name="manage-teams-apps-in-the-microsoft-teams-admin-center"></a>Microsoft Teams 管理センターで Teams アプリを管理する

Teams 管理センター ポータルの **[Teams アプリ** ] ページで、組織のアプリを管理します。 [アプリの管理] ページを使用すると、組織のアプリ カタログ内のすべての Teams アプリを表示および管理したり、アプリ管理の際立つユース ケースに対応したり、ポリシーを使用してアプリへのアクセスを定義したりできます。
...
Teams 管理センターを使用するには、グローバル 管理または Teams 管理者ロールが必要です。 詳細については、 [Teams 管理者ロール](./using-admin-roles.md) と [Microsoft 365 管理者ロールに関するページを](/microsoft-365/admin/add-users/about-admin-roles)参照してください。
...
[アプリの管理] ページは、組織レベルで個々のアプリを許可またはブロックする場所です。 ページには、使用可能なすべてのアプリとその現在の組織レベルのアプリの状態が表示されます。 アプリの一覧には、Microsoft、サードパーティの開発者、および組織内の開発者によって提供されるアプリが含まれます。

アプリを許可またはブロックするには:

1. Teams 管理センターにサインインします。
1. **Teams アプリ** > の **[[アプリの管理]](https://admin.teams.microsoft.com/policies/manage-apps)** ページにアクセスします。
1. アプリの一覧からアプリを選択します。 アプリの名前で検索し、それを選択できます。
1. [ **許可]** または [ **ブロック]** オプションを選択します。

Teams 管理センターの [ [アプリの管理](https://admin.teams.microsoft.com/policies/manage-apps) ] ページでアプリを許可 (またはブロック) すると、組織内のすべてのユーザーに対して特定のアプリが許可 (またはブロック) されます。 この方法は、アクセス許可ポリシーを使用してアプリを許可 (またはブロック) すると、ポリシーが割り当てられた特定のユーザーにのみ影響を与えるコンテキストのアプリアクセス許可ポリシーとは異なります。

ユーザーは、テナント全体の設定でアプリが許可され、アクセス許可ポリシーを使用してユーザーに許可されている場合にのみ、アプリをインストールして使用できます。
...
## <a name="allow-the-apps-that-are-blocked-by-the-developers"></a>開発者によってブロックされているアプリを許可する

開発者が Teams アプリ ストアにアプリを発行するときに、アプリ エクスペリエンスを構成またはカスタマイズするために管理者が必要になる場合があります。 管理者は、アプリのセットアップ時にエンド ユーザーがエクスペリエンスを利用できるようにします。

たとえば、Contoso Electronics は、Microsoft Teams 用のヘルプ デスク アプリを作成した ISV です。 Contoso Electronics は、その顧客がアプリを操作するときに期待どおりに機能するように、顧客がアプリの特定のプロパティを設定することを望んでいます。 管理者がアプリケーションを許可またはブロックする前に、Teams 管理センターで **公開元によってブロック** として表示され、既定ではエンド ユーザーから非表示になります。 アプリを設定するための発行元のガイダンスに従った後、状態を **[許可]** に変更することで、ユーザーがアプリを使用できるようにします。

:::image type="content" source="media/blocked-by-publisher.png" alt-text="Teams 管理センターの発行元の状態によってブロックされたスクリーンショット。":::
