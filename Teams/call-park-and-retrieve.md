...
カスタム のパーク範囲とパーク タイムアウトを設定するには、Teams PowerShell モジュール 2.6.0 以降で使用できる New- コマンドレットと Set-CsTeamsCallParkPolicy コマンドレットを使用します。 (カスタム パーク範囲とパーク タイムアウトの変更は、管理センター Teamsできません。 管理センター Teams既定値が引き続き表示されます)。
