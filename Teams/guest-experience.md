...
Q。 来園者にはチーム メンバーと同じ機能がありますか。A: いいえ 詳細については、どのようなゲストができるし、実行できない、チームでのゲストの機能を参照してください。

Q。 どのように多くの来園者を招待できますか。A. ライセンスを付与されたユーザーごとの 5 つのゲストまで追加できます。

Q。 来園者にはビジネスの OneDrive へのアクセスがありますか。A: いいえ

Q。 来園者を持ってアクセスする SharePoint ファイル?A: はい。

Q。 来園者は、ファイル内で検索できますか。A: いいえ

Q。 来園者がファイルを添付することができますか。はい、ゲストが次の 2 つの方法でファイルを関連付けることができます: • が左側のウィンドウでファイルを選択し、ファイルの場所を参照します。
• 自分のコンピューターからアップロードします。

Q。 ゲストはプライベート チャットにファイルをダウンロードできますか。A: はい、秘密のチャットでのメンバーからファイルを受信し、自分のデスクトップにダウンロードすることにできます。
