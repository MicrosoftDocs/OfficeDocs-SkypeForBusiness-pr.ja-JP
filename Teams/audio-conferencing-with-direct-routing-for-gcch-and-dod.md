...
- GCC[高]: 電話会議 - GCCの高テナント ライセンスと電話会議 - GCCユーザーの高いライセンス。
...
> [!IMPORTANT]
> ダイヤルイン電話番号が設定されるまで、ユーザーは直接ルーティングを使用した電話会議を有効にすることはできません。 この記事で説明するダイヤルイン電話番号を設定するまで、GCC High または DoD ライセンスの直接ルーティングを使用した電話会議をユーザーに割り当てないでお勧めします。  このガイダンスに従う必要が生じない場合は、ダイヤル パッドがクライアントのTeamsがあります。
...
> [!NOTE]
> Location-Based Microsoft 365 Government Community Cloud (GCC) High デプロイまたは DoD デプロイでは、ルーティングを使用できません。 電話会議を有効にする場合は、GCC High または DoD 環境の電話会議ユーザーが会議ルーティングに対して有効になっていないことをLocation-Basedしてください。
...
音声ルーティング ポリシーで複数の PSTN 使用法が定義されている場合は、定義された順序で評価されます。 PSTN の使用に関連付けられている音声ルートの番号パターンの観点から、PSTN の使用を最も一般的な順序で定義してください。 たとえば、PSTN の使用が米国への通話をルーティングするために定義され、別の PSTN 使用法が世界中の他の場所に通話をルーティングするために定義されている場合、通話を世界中の他の場所にルーティングするために PSTN の使用の前に、米国への通話の PSTN 使用法が音声ルーティング ポリシーに一覧表示される必要があります。
...
- 設定が変更された場合にユーザーに通知メールを送信する。 電話会議通知メールは、高レベルおよび DoD の直接ルーティングを使用した電話会議GCCサポートされていません。
