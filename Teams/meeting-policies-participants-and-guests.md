...
この設定は、リーダーレスダイヤルイン会議を可能にする開催者ごとのポリシーです。 この設定は、組織から認証されたユーザーが参加していない場合でも、ダイヤル インのユーザーが会議に参加できるかどうかを制御します。 既定では、この設定はオフになっています。つまり、ダイヤルイン ユーザーは、組織の認証されたユーザーが会議に参加するまでロビーで待機します。
...
|**自分とゲストの組織のユーザー**     |ゲスト ユーザーを含む組織内の認証済みユーザーは、ロビーで待機せずに直接会議に参加します。 信頼できる組織のユーザーと匿名ユーザーはロビーで待機します。 これは、既定の設定です。    |
...
|**招待されたユーザーのみ**    |ロビーで待機せずに会議に直接参加できるのは、招待されたユーザーと会議の開催者のみです。 組織内の認証済みユーザー、ゲスト ユーザー、信頼できる組織のユーザー、匿名ユーザーなど、他のすべてのユーザーはロビーで待機する必要があります。 [Teamsの会議オプション] ページに、[招待するユーザー] と表示されます。 配布グループの一部として追加されたユーザーは、ロビーを通過する必要があります。      |

 > [!NOTE]
> 信頼できる組織は、フェデレーション通信を許可するドメインTeams。 Teams 管理センターで [すべての外部ドメインに外部アクセスを許可する] を有効にした場合、組織内のすべての認証済みユーザー Teams信頼されます。 許可されている外部ドメインを指定し、他のすべてのドメインをブロックすると、許可されているドメインは信頼できる組織になります。 ブロックされているドメインは、信頼できる組織ではないと見なされます。
...
|値を設定する |動作  |
|---------|---------|
|**有効**     | すべての参加者がチャット メッセージを書き込み、表示できます。 |
|**無効**     | 会議チャットは、すべての参加者に対して無効になります。  |

<a name="bkparticipantsandguests"> </a>

## <a name="enable-meeting-policy-settings"></a>会議ポリシーの設定を有効にする

会議ポリシー設定を有効にするには、Teams 管理 **センター (** >  > [会議ポリシー]ポリシーを編集するポリシー **を編集する**& ゲスト)、または [Teams](https://admin.teams.microsoft.com/policies/meetings) PowerShell の [Set-CsTeamsMeetingPolicy](/powershell/module/skype/set-csteamsmeetingpolicy) コマンドレットを使用できます。 

この例では、PowerShell を使用して、すべてのユーザーが会議を開始または参加できるグローバル会議ポリシーを変更します。
...
ポリシーを設定したら、ポリシーをユーザーに適用する必要があります。 グローバル (組織全体の既定) ポリシーを変更した場合は、自動的にユーザーに適用されます。 ポリシーの変更が有効なまで少なくとも 4 時間待つ必要がありますが、最大 24 時間かかる場合があります。
