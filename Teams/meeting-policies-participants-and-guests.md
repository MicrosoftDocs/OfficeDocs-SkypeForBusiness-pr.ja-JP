...
|**招待されたユーザーのみ**    |ロビーで待たずに直接会議に参加できるのは、招待されたユーザーと会議の開催者のみです。 組織内の認証されたユーザー、ゲスト、信頼できる組織のユーザー、匿名ユーザーなど、他のすべてのユーザーはロビーで待機する必要があります。 Teams クライアント会議のオプション ページで、"招待するユーザー" と表示されます。 配布グループの一部として追加されたユーザーは、ロビーを通過する必要があります。      |
...
この設定は、チャネル会議には適用されません。 この会議チャット ポリシーがユーザーに適用されると、開催者は会議オプションを使用してこのポリシーをオーバーライドできません。
