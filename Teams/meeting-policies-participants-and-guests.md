...
|**自分とゲストの組織のユーザー**     |組織内の認証されたユーザー (ゲスト ユーザーを含む) は、ロビーで待機することなく直接会議に参加します。 信頼できる組織のユーザーと匿名ユーザーはロビーで待機します。 これは、既定の設定です。    |
|**自分の組織、信頼できる組織、およびゲストのユーザー**     |ゲスト ユーザーや信頼された組織のユーザーなど、組織内の認証ユーザーは、ロビーで待たずに会議に直接参加します。  匿名ユーザーは、ロビーで待機します。   |
|**自分の組織のユーザー**    |組織内の認証されたユーザーは、ロビーで待機することなく直接会議に参加します。  信頼できる組織、ゲスト ユーザー、匿名ユーザーのユーザーはロビーで待機します。          |
|**開催者のみ**    |会議の開催者のみが、ロビーで待たずに会議に直接参加します。 組織内の認証されたユーザー、ゲスト ユーザー、信頼できる組織のユーザー、匿名ユーザーなど、他のすべてのユーザーはロビーで待機する必要があります。 [Teamsクライアント会議オプション] ページに、"自分のみ" と表示されます。          |
|**招待されたユーザーのみ**    |ロビーで待たずに直接会議に参加できるのは、招待されたユーザーと会議の開催者のみです。 組織内の認証されたユーザー、ゲスト ユーザー、信頼できる組織のユーザー、匿名ユーザーなど、他のすべてのユーザーはロビーで待機する必要があります。 [Teamsクライアント会議オプション] ページに、"招待するユーザー" と表示されます。 配布グループの一部として追加されたユーザーは、ロビーを通過する必要があります。      |

 > [!NOTE]
> 信頼できる組織は、Teamsでのフェデレーション通信を許可するドメインです。 Teams管理センターで **外部アクセスに対するすべての外部ドメインを許可** するを有効にすると、任意のTeams組織内のすべての認証されたユーザーが信頼されます。 許可される外部ドメインを指定し、他のすべてのドメインをブロックすることを選択した場合、許可されたドメインは信頼できる組織になります。 ブロックされたドメインは、信頼できる組織でないと見なされます。

## <a name="allow-dial-in-users-to-bypass-the-lobby"></a>ダイヤルイン ユーザーによるロビーのバイパスを許可する

これは開催者単位のポリシーです。 この設定では、スマートフォンでダイヤル インするユーザーが会議に直接参加するのか、[**ユーザーの参加を自動的に許可する**] の設定に関わらずロビーで待機するのかを制御します。 既定では、この設定は [オフ] です。 この設定をオフにすると、組織のユーザーが Teams クライアントで会議に参加し、そのユーザーに入室を許可するまで、ダイヤルイン ユーザーはロビーで待機します。 この設定をオンにすると、組織ユーザーがTeams クライアントを使用して会議に参加すると、ダイヤルイン ユーザーは会議に自動的に参加します。
...
|値を設定する |動作  |
|---------|---------|
|**有効**     | すべての参加者は、チャット メッセージを書き込んで表示できます。 |
|**無効**     | 会議チャットは、すべての参加者に対してオフになっています。  |

<a name="bkparticipantsandguests"> </a>

## <a name="enable-meeting-policy-settings"></a>会議ポリシー設定を有効にする

会議ポリシー設定を有効にするには、[Teams管理センター](https://admin.teams.microsoft.com/policies/meetings) (**Meeting PoliciesEdit** >  **a** **policyParticipants** >  &ゲスト) または PowerShell の [Set-CsTeamsMeetingPolicy](/powershell/module/skype/set-csteamsmeetingpolicy) コマンドレットTeams使用できます。 
