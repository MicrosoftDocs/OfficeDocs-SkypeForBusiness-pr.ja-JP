...
|値を設定する |動作  |
|---------|---------|
|**すべてのユーザーに対して有効にする**     | すべての参加者がチャット メッセージを書き込み、表示できます。 |
|**すべてのユーザーに対してオフにする**     | 会議チャットは、すべての参加者に対して無効になります。  |
|**匿名ユーザーを含むすべてのユーザーに対して有効にする**| 匿名ユーザーは会議中にチャットできないが、メッセージを読み取る。 |

<a name="bkparticipantsandguests"> </a>

## <a name="enable-meeting-policy-settings"></a>会議ポリシーの設定を有効にする

会議ポリシー設定を有効にするには、Teams 管理 **センター (** >  > [会議ポリシー]ポリシーを編集するポリシーを編集 **する**& ゲスト)、または [Teams](https://admin.teams.microsoft.com/policies/meetings) PowerShell の [Set-CsTeamsMeetingPolicy](/powershell/module/skype/set-csteamsmeetingpolicy?view=skype-ps) コマンドレットを使用できます。 
